...
| UNK-11 | Opponent AI route choice/difficulty model. The roster side is now verified (RACE-11/RACE-12): which aimap variant binds, which vehicle/route each row wires, and how the wired count relates to the table `Opponents` column. Still unverified: `.opp` column semantics (`brake`, `forward/side offset`, `target speed`, `speed/side start` — all preserved raw), the 10-value `[Opponent]` parameter tail (first behaves like a 0-1 skill), how the original consumes the driving line vs its own route choice (CHK-5 says opponents take different routes), how opponents take grid slots (UNK-17), and what the spare `.opp` files were for. |
