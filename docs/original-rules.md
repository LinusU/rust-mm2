...
| UNK-22 | Banger runtime semantics (WLD-15): what `ImpulseLimit2` is compared against and what crossing it does (break vs. tip vs. nothing), whether `NumParts` bounds spawned fragments or only echoes the PKG's BREAK count, the `ColliderId`/`AudioId`/`TexNumber` id spaces, `SpinAxis`/`BillFlags`/`Flash`/`YRadius` semantics, the `type: a` tag, when `BirthRule` fires relative to breakage, how `default.dgbangerdata` fallback selection works, and whether the 4 fragment records carrying `NumParts>0` mean fragments-of-fragments or inert leftovers. The record↔geometry link is verified; the state machine is not. |
