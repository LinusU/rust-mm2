...
| F13-C | active | F13-B, F15-B | Split into C.1 (catalog × runtime × opponents matrix — implemented below). Remaining: Professional-difficulty runtime leg, a true stationary-player isolation driver (the `Hold` leg drives blind at full throttle), diagnosing sf-8's ~8-12 updates/s CPU-bound leg, longer budgets for the deep London/SF courses, and original-fidelity comparisons (difficulty, pacing, AI competence vs retail are unverified — engine self-metrics only). |
