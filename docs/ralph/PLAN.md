...
remaining box-yield/right-of-way/collision scope** — the
...
and the original-menu audit against F17-AC05's capability
...
| F10-B | active | F10-A | B.1 obstruction response + B.2 authored junction rules + B.3 bounded stuck recovery + B.4 spawn occupied-space rejection implemented above. Remaining: right-of-way/yielding between vehicles inside the junction, queue-through-intersection priority, multiplayer union-of-interest bubbles, signal-prop rendering, collision fidelity (AC03), original junction/spawn timing (UNK-12), F10-AC evidence vs the spec. |
| F10-C | queued | F10-B | — |
...
| F17-A | active | F01-A, F02-A, F11-A, F16-A | Split: A.1 (menu shell — profile/mode/content selection over the real catalogs), A.2 (Quick Race — DRV-8's `last_event` launch), and A.3 (driver-name text entry), all implemented below. Remaining: per-event weather/time/density controls (need F18's session-legal writers; RACE-3 `customizable`), mouse navigation, original menu audit vs F17-AC05's capability denominator. |
