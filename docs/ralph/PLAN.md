...
| F10-A | active | F01-B, F02-A, F09-B | Split into A.1 (ambient-traffic catalog + seeded spawn-policy planner — implemented below). Remaining: session-scoped ambient entities (spawn/despawn consumption, lane-following kinematics, collision), F10-AC evidence vs the spec. |
