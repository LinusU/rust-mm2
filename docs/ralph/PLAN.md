...
| F18-A | active | F01-B, F06-B | Split: A.1 (preset inventory — implemented below), A.2 (preset→Bevy lighting binding + session-legal selection — implemented below), A.3 (`_fog.csv` per-preset fog tables → `DistanceFog` — implemented below), A.4 (`.sky` dome consumption — implemented below). Remaining: `.cpvs` variant selection/PVS culling, `.ldef`/`.pvshist`/`.lmap`/`.water` consumers, precipitation/wetness/audio and condition replication (F18-B/C scope — see UNK-24). Menu weather/tod selection landed in F17-A.6 through `SessionConfig::customization`. |
