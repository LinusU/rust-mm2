...
| F13-C | active | F13-B, F15-B | Split into C.1 (catalog × runtime × opponents matrix — implemented below) and C.2 (stationary-control driver + first Professional legs — implemented below). Remaining: the rest of the Professional matrix (21 events undriven at Pro; no Pro hold/parked legs), diagnosing sf-8's ~8-12 updates/s CPU-bound leg, longer budgets for the deep London/SF courses, and original-fidelity comparisons (difficulty, pacing, AI competence vs retail are unverified — engine self-metrics only). |
