...
detachment, impairment, gyro consumption, water recovery,
...
| F05-B | active | F05-A | Split: B.1 (runtime impact→damage application + disabled outcomes) and B.2 (authored `vehstuck` detection + in-place recovery) implemented below. Remaining: visual tiers (smoke pivots, `TextelDamageRadius` decals, `DoublePivot`/`MirrorPivot`), breakaway detachment lifecycle (REC-1 inventory exists, no part detaches), impairment short of destruction, `vehgyro` consumption, water/out-of-bounds recovery, C&R healing driver (DMG-4), replication. |
