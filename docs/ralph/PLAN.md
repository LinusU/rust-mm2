...
(impacts/skid/clutch/siren + audible evidence))** — the latest
iteration landed F07-B.2: opponent engine rigs + the spatial
...
| F07-B | active | F07-A | Split: B.1 = RPM-driven engine loop rig for the local player (candidate at `280edb9`, pending external check), B.2 = opponent rigs + spatial emitters + camera listener (below). Remaining: impact/scrape voices off the deduplicated `ImpactEvent` stream (`default_impacts` force bands — binding UNK-25), surface skid/rolling audio (`default_surface*` band schemas — UNK-25), ambient-traffic engines (different cardata grammar — `AmbientEngine`), clutch sample trigger, siren programs, AC03/AC04/AC05 negative + audible evidence. |
