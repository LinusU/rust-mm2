...
| F07-B | active | F07-A | Split: B.1 = RPM-driven engine loop rig for the local player (below). Remaining: opponent/ambient engines + spatial mix + listener, impact/scrape voices off the deduplicated `ImpactEvent` stream (`default_impacts` force bands — binding UNK-25), surface skid/rolling audio (`default_surface*` band schemas — UNK-25), clutch sample trigger, siren programs, AC03/AC04/AC05 negative + audible evidence. |
