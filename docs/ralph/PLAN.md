...
implemented F18-A.2: the measured `.ltNN` presets now bind to
...
| F18-A | active | F01-B, F06-B | Split: A.1 (preset inventory — implemented below), A.2 (preset→Bevy lighting binding + session-legal selection — implemented below). Remaining: `.sky` dome/fog mapping, `.cpvs` PVS consumption, menu weather/tod selection, precipitation/wetness/audio and condition replication (F18-B/C scope — see UNK-24). |
