...
remaining right-of-way/collision scope** — the
...
| F10-B | active | F10-A | B.1 obstruction response + B.2 authored junction rules + B.3 bounded stuck recovery + B.4 spawn occupied-space rejection + B.5 junction-box yield implemented above. Remaining: queue-through-intersection priority, multiplayer union-of-interest bubbles, signal-prop rendering, collision fidelity (AC03), original junction/spawn timing (UNK-12), F10-AC evidence vs the spec. |
| F10-C | queued | F10-B | — |
