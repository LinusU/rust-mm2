...
**Next selected slice: F05-B remainder (visual tiers, breakaway
detachment, impairment, stuck/gyro consumption, water recovery,
C&R healing driver), F17-B remainder (C&R/scoring variants — need
...
latest iteration landed F05-B.1, runtime impact→damage
...
| F05-B | active | F05-A | Split: B.1 (runtime impact→damage application + disabled outcomes — implemented below). Remaining: visual tiers (smoke pivots, `TextelDamageRadius` decals, `DoublePivot`/`MirrorPivot`), breakaway detachment lifecycle (REC-1 inventory exists, no part detaches), impairment short of destruction, `vehstuck`/`vehgyro` consumption, water/out-of-bounds recovery, C&R healing driver (DMG-4), replication. |
