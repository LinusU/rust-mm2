...
`avoidOpponents` polarity — now incl. the steep-hillside
drivability wall the routed scripted player and the AI field
both hit on `sf circuit:0`'s p3→p4 descent),
...
| F15-B | active | F15-A | Split into B.1 (participant avoidance/overtake), B.2 (authored `[Opponent]` param-tail decode/consume — checked at `a203f833`), B.3 (bounded re-anchor recovery), B.4 (designed catch-up assist + `cu=` disclosure — implemented below), B.5 (scripted-player route guidance + bounded re-anchor — implemented below). Remaining: AC06's measured difficulty effects, representative avoidance matrix, `weirdPathfinding`/`distancePadding`/`cornerBrakingThreshold` consumption once semantics verify, `avoidOpponents` polarity (inert meanwhile), and the steep-hillside drivability wall on `sf circuit:0`'s p3→p4 descent that both the routed bot and the AI field hit (physics/collider-fidelity scope, not routing). |
