...
**Next selected slice: F01-C** — session start/quit/restart integration
in the real app plus plugin-attach documentation (see task table).
F01-A is externally checked; F01-B is implemented as a candidate:
stable ids, telemetry, surface/impact/result contracts and local
authority boundaries in `mm2_game`, bridged from real Avian contacts in
`mm2_app`.
