...
needs F27's mode; replication — F25+), F11-C remainder
(AC02–AC05 promotion — no new runtime work named), F13-C
(original-fidelity comparison), F17-B remainder (C&R/scoring
result variants — needs F17-C's mode),
the F18-A remainder (→ F18-B/C scope), the F22-A remainder
(HUD-2 race instruments over the dev line; A.1 landed the authored
minimap, A.2 the `I` indicators, A.3 the `H` master toggle, A.4 the
authored stopwatch/countdown timer — checkpoint list/lap/place
instruments stay open), F17-A remainder
(per-event weather controls landed in A.6 via F18; AC03 evidence
leg — interactive), F16-C's AC01
process-level leg (interactive finish), F10-B's remaining AC03 checklist scope
(player-hit feel leg — manual), the F07-B remainder
(scrape + audible
...
semantics unverified))** — the latest iteration landed F22-B.4, the
F22-B AC05 atypical-vehicle leg: the `CollideType` occlusion ray
counted the player's own trailer as world geometry (vpsemi's authored
near boom lands *inside* its ~14 m trailer box → near view parked in
the hitch gap on a cab-rear close-up, far view clamped behind the
trailer wall — both verified broken on windowed retail captures and
fixed by excluding `SpawnPoint.trailers` from the ray; other trailers
still occlude, UNK-36), and four `_dash.campovcs` records
(vpsemi/vpcentury/vpcoop2k/vpvw_dune) author `CameraNear 3.0` which
clipped the whole interior cluster ~1 m ahead of the eye — the
cockpit camera now caps it at a designed 0.5 m (authored value still
reaches the mirror camera; UNK-37), verified by windowed vpsemi/
vpcentury cockpit captures now showing the full authored dashes. The
review's two missing camtrack test legs (far-mode driving gate,
sized-lens boom) were added for real, and windowed retail captures
cover vpbus near/far, vpcentury/vpmoonrover near, a vpsemi mirror
strip and a dev-world wall pull-in. Before that the latest iteration repaired the F21-A.1
review's blocking finding (iteration 73): the research doc's
falsified retail measurements were corrected against the committed
tool's own output (four sf lessons share one identical ten-road
`[Exceptions]` block, not `crash12` alone; `tail[1]`=1 on the two
`crash4` lessons; the `stop`/`exam1_2` rows carry `tail[2]`/`numopp`,
not `tail[3]`/`tail[4]`), and `CrashDataFile::diagnostics` now ride
`LessonTable` into the audit so a dropped row can't shrink it
silently. Before that it landed F21-A.1 (iteration 72: the Crash
Course lesson catalog audit — `mm2_content::crashcourse` +
`mm2-inspect crash-course`; F21-A stays `active` pending evaluator
work), F22-A.6 (71: the standings cluster) and F22-A.5 (70: the
authored nav arrow). Before those it landed F22-A.4
(iteration 69): HUD-2's stopwatch/countdown pair — the `mmTimer`
instruments the recovered `mmHUD` layout owns — rendered from the
authored `digitac_*`/`digi_colon` glyph set bound through the VFS
(22 stems; `absent:missing-glyphs` on the report, never substitute
art). `mm2_app::racetime` composes a designed `m:ss:hh` row
top-centre under the nav arrow — full-size digits for
minutes/seconds, the authored half-size set for centiseconds,
leading-zero suppression capped at `999:59:99` (DSN-53/UNK-32) —
driven off authoritative `RaceState`: `time_remaining` on timed
(Blitz) definitions, `clock` otherwise, frozen by pause/finish,
armed during `Countdown`, dark on `Complete`/stale generations, and
gated by `H` while the report keeps composing (`tmr=` records the
display anyway, like `ind=`'s `bound`). `camera::retarget_hud`'s
`HudNodes` gained the timer — plus `NavArrow`/`LowTimeWarning`, which
without a `UiTargetCamera` fell back to the *highest-order* primary
camera (the mirror strip, order 2) and rendered inside it whenever
armed; the `LOW TIME` cue moved below the timer plate. Headless
record: `tmr=<glyphs>g/<display>` on event sessions only. Verified
by 13 new racetime tests (slot composition, full/partial/missing
binding, count-up/down, countdown arming, `Complete`/stale release,
`H` gate, synthetic + retail event legs) and windowed captures on
retail sf (checkpoint run `0:04:44`, blitz countdown `0:30:00`,
cockpit retarget `0:02:73`, mirror-strip overlap documented). Before
that it landed F22-A.3
(iteration 68): the documented `H` HUD toggle (HUD-3/CTL-1). Exe
evidence (mm2hook `mmHUD`/`mmHudMap` layouts) shows the original's
HUD as a single node owning `mmHudMap` (itself drawing the opponent
indicators via `DrawIndicator`), `mmArrow`, the `mmTimer` pair,
`mmDashView` and `mmCRHUD` with `Enable`/`Disable`/`Toggle` — so `H`
is a master gate over the whole driving-HUD layer. `mm2_app::hud`
holds the session-agnostic `HudVisible` (on by designed default),
`hud_input` (`H` in `Playing`/`Countdown` only) and `update_hud`
(moved off the bin target); every layer member ANDs the gate onto its
own visibility write while keeping authoritative state — instrument
line, nav arrow, countdown/low-time cues, map corner views, opponent
indicators (`bound` still counts demand), cockpit cluster (the
cockpit camera itself keeps rendering). Outside the layer by design:
the mirror strip, nav debug, `ErrorText`, session/menu overlays and
the full-screen pause map — DSN-52; the original's exact flag scope
and whether the mirror suppresses stay unrecovered (UNK-31).
`--no-hud` starts the layer off (render-only); headless records
report `hud=off` on-activity only. Verified by 8 new hud tests (phase
gating, every member's hide/restore, pause-map + `ErrorText`
exemptions, demand bookkeeping, teardown survival, `hud=off` record
leg + default-record absence). Before that it landed F22-A.2
(iteration 67): the documented `I` opponent indicator (HUD-3/CTL-1) —
a session-owned pool of authored `hudmap_tri` markers riding each
non-local `Player` participant's roof (per-frame entity-order
binding, stale slots freed same update, `I` in live phases only,
session-agnostic toggle; presentation designed — DSN-51/UNK-30),
`ind=<on|off>/<pool>m/<bound>b` headless, verified by 5 oppind tests
plus retail sf checkpoint:0 `ind=on/6m/6b` and a windowed capture.
Before that it repaired
the F22-B.2 review's single blocking finding (iteration 66): `retarget_hud` was
...
`camPovCS` eye (designed `chassis`-height fallback), rendering a
top-centre `Viewport` strip (⅓ × ⅛ physical window, designed geometry —
the original's mirror presentation is unrecovered, DSN-50/UNK-29) and
toggled by `mirror_input` in `Playing`/`Countdown` only so overlays
keep the key; `RearView` is session-agnostic so a restart respawns the
strip armed, `WorldCamera3d` excludes it from every "the active
camera" pick (listener/PVS/dome/pose/billboards), `sync_dash_visibility`
exempts it so the strip stays up over the cockpit view, and the dev
build's borrowed BACKSPACE restart moved to the documented `F4`
binding. `--mirror` arms it for frozen-input captures; headless records
report `mir=on|armed`. Verified by 7 new mirror tests + a session
restart-binding test, retail sf headless `mir=on`, and a windowed
capture showing the rear view in the strip with the HUD `cam` pose
still on the chase camera. Before that it repaired the F22-B.1
review's new blocking finding (iteration 64): the authored cockpit
camera is the codebase's first child-of-vehicle camera, and three
"active camera" consumers read its car-local `Transform` as a world
pose — `drive_sky_dome` parked the 900 m dome near the world origin
under `CameraMode::Cockpit` (a clear-colour sky across most of each
city), `active_cam_pose` reported car-local coordinates (breaking the
HUD `cam` readout, screenshot filenames and the `--cam` round-trip),
and `apply_city_pvs` fed a bogus source position near the origin
(over-show only). All three now read the active camera's
`GlobalTransform` — the `damage_fx` billboard query's precedent —
`active_cam_pose` moved into `mm2_app::camera` for coverage, and the
two remaining `Without<HudMapCamera>` camera picks tightened to the
`WorldCamera3d` alias so a stray active menu `Camera2d` can never be
the world view. Regression tests put a vehicle-child active camera
through all three consumers (dome recentre, PVS source room, pose
string); a windowed `--cockpit --spawn=-1300,63.5,250,0` capture
renders the authored dome 1.3 km from the origin with a world-space
`cam` readout. Before that it repaired the same review's two earlier
blocking findings (iteration 63): the
`sync_dash_visibility` sweep wrote `Visible` onto every non-cockpit
vehicle child every non-Cockpit frame, clobbering `Hidden` states
owned by `detach_breaks`/`restore_rig` (a detached panel re-rendered
attached beside its own fragment) and `update_glows` (unlit glow
quads); and the `--cockpit`-on-a-dashless-car/dev-world/Cockpit-
persisted-across-reload fallback left `CameraMode::Cockpit` with zero
active cameras (dead render). `sync_dash_visibility` now tags what it
hides (`CockpitHidden`) and restores tagged children only — glow
carriers are never tagged (`update_glows` re-derives them per frame,
and the split is ordered after it) and `detach_breaks` drops the tag
when it claims a node's `Hidden`; the effective camera mode resolves
before the session cameras spawn (`load_pov_cam` probed first, pov
handed to `spawn_dash`), so Cockpit without an authored `camPovCS`
falls back to an *active* chase camera — and a `C` press with zero
marked session cameras (menu phase) is now a no-op instead of
drifting the mode. Before that it landed F22-B.1,
...
is `Paused` or a pause intent is pending, and both pause dev flags
carry the MP-6 `allows_pause` gate). Before that it landed F22-A.1,
opening the F22-A parent: the authored in-race HUD minimap —
`tune/<city>.mmhudmap` parse (the recovered `mmHudMap` tune),
`geometry/hudmap_<city>.pkg` world-space tiles under a dedicated
orthographic map camera/layer, authored `hudmap_tri`/`hudmap_square`
markers for player/opponents/checkpoints/finish, the documented
TAB/E/F/Q controls (HUD-4) including the single-player fullscreen
pause map that replaces the pause overlay, and a `map=` smoke field —
verified headlessly on both retail cities and with windowed
screenshots (inset + fullscreen). Before that the latest
iteration landed F14-C.1,
opening the F14-C parent: the `--restart-at` dev override (a
tick-gated `--restart` so a run banks real progress before the
teardown) plus retail legs — mid-race restarts on sf-0 and
london-0 (`rs=1`, gen-2 counters fresh, a generation-scoped
opponent finish minting `results=1`), the finish-line-spawn edge
on authored geometry (parked inside sf-0's closing-gate cylinder
the whole run: `cp=0/9 lap=1/3`), and the first retail Ordered
resolution (`--finish` on sf-0 → `phase=results`, `lap=3/3` closed
by the lifted start-line gate). Before that the latest
iteration landed F14-B.2, closing the F14-B parent: the Ordered
lap-validation edge coverage
— the spec's edge list (finish-line spawn, overlapping
start/finish volumes, skipped gate, last-lap tie, DNF participant,
reset on finish) exercised through the production
`advance_race`/`reanchor_teleported_participants` path, +7 tests in
`tests/race.rs` (36→43), no production-code delta — the `Ordered`
contract already enforced every case; the slice adds the evidence
and the parent promotion. Before that the latest
iteration landed F14-A.6,
closing the F14-A parent: AC05's Ordered restart leg
(`restart_restores_the_circuit_grid_counters_and_objects` — a
lapped, rostered, gridded circuit event banked mid-race then
restarted through the production teardown path; grid, counters,
clock, route binds, markers and ledger all verified restored/scoped
for generation 2) plus the AC promotion record — all six F14-A ACs
now carry evidence and the parent row reads `implemented`
(candidate pending external check). The same iteration committed
the preserved iteration-56 work (`204373d`). Before that the latest
iteration landed F14-A.5, the
...
Pro measurably selects `.aimap_p` rosters and authored lap counts;
the first catalog-wide run with DSN-45 route credit live shows `/Nd`
on 14/20 events and `0d` exactly at the traversal residuals
(london-2/5, sf-7 pack — fields never reach the binds). Multi-lap
churn is real at Pro: opponents complete ≥1 lap on 10/20 events,
london-0's whole 7-car field laps (16/18 completions, four cars
reached lap 4) and one `vpcoop2k` finished `6c/4l/F` — the first
Professional circuit finish, minting exactly one ledger result while
the local raced on. Published in `docs/race-coverage.md`; candidate
pending external check. The same iteration repaired the F14-A.3
review's doc finding — the `tests/race.rs` increment is 28 → 35
(+7), the "+9" figure only true across both suites (+2 in
`tests/opponents.rs`). Before that the latest iteration landed
F14-A.3, the route-bound Ordered AI progress model (DSN-45) that
closed the authored-miss defect class — see its row; before that
F14-A.2, the Amateur Circuit matrix + the `densify_route`
gate-coverage repair its analysis surfaced. Before that
the latest iteration landed F10-B.15, the
multi-edge collision-accounting repair closing the symmetric-edge
gap the B.14 review disclosed: striker corrections were per-edge
*velocity targets*, so a striker contacting two cars in one drain
had its second write erase the first edge's payment (both cars
launched on one transfer — momentum injection), and a striker the
same pass itself flipped was skipped outright (a daisy chain's
last car launched uncharged). `knock_ambient` now applies flips
first (each `Knock` captures `struck_pre`), then compounds striker
corrections: first committed edge = the wall-returning velocity
target, every later edge = a pure `−dir·J` debit; a car flipped on
a *different* pair pays the debit while the follower-follower
mutual pair still owes nothing (its struck-side launch is its
share). Four new `Collisions`-proven integration tests cover the
one-striker-two-cars drain, the same-tick dynamic→lane→lane daisy
chain, the mutual follower-follower pair, and the 3-striker
pileup. Retail headless records bit-identical to B.14. Before that
the latest iteration landed F10-B.14, the
striker-correction spin bound + same-tick pileup coverage, both
repairing gaps the B.13 review named on the same handover:
`write_striker_correction` (the shared helper the banger and both
ambient striker paths funnel through) now clamps the correction's
post-write spin at `MAX_BANGER_ANGULAR_SPEED` — a `Player` striker
carries no solver bound at all, so the write clamp is its only
one — and `a_same_tick_pileup_flips_the_car_once` proves two
strikers contacting one lane car on the same update (verified
through the `Collisions` graph) produce one flip, one `x` charge,
a single transfer's launch, and the winner/loser split (corrected
~6 m/s share vs uncorrected ~14 m/s wall shove). Retail headless
records bit-identical to B.13 — the bound never engages at
ordinary speeds. Before that the latest iteration landed F10-B.13, the
wreck bound + striker-class disclosure on the same handover:
ambient cars now carry the solver-side
`MaxLinearSpeed`/`MaxAngularSpeed` every banger body gets (inert
while kinematic, binding once flipped) and the contact-lever spin
write clamps at `MAX_BANGER_ANGULAR_SPEED`, closing the
external review's named gap — a transient spike could leave a
fast-spinning wreck whose inflated `normal_speed` fed later
contacts, the same cascade class sf-8 hit. Each handover also
counts its striker's class (`Player` participant / ambient car /
other) into the record's new `kns=` field — the same review
could not tell whether a participant ever struck a car. Retail
headless (`fnv1a64:e91e6cd4b2ae30d9`): sf `--frames 3000` →
`kn=4 kns=2p/2a/0x` (two real participant strikes, `dmg=23a`
through the impact→damage chain), london `--frames 1200
--spawn 0.4,5.5,-720,0` → `kn=2 kns=0p/0a/2x` (both `x` —
neither a participant nor an ambient car; banger bodies and
break fragments are the remaining class). Before that
the latest iteration landed F10-B.12, the
momentum-correct ambient-collision handover: B.6's bounded kick
on top of the solver's infinite-mass wall answer injected energy,
so the flip now replays the hit as the shared two-body transfer
(`(1+e)·v·μ` — the same `contracts` math banger activation runs,
extracted from `banger.rs`), the striker's push-direction
component is rewritten to its exchange share as a velocity target
(Avian's recorded `total_impulse` accumulates recovery and
restitution passes — it over-reports the real Δv, so the target
is reconstructed from measured severity instead), contact-lever
spin applies to the wreck, and a follower-follower edge never
charges the exchange twice. Retail headless
(`fnv1a64:e91e6cd4b2ae30d9`): sf `--frames 3000` → `kn=4`,
london `--frames 1200 --spawn 0.4,5.5,-720,0` → `kn=2` — real
handovers on both cities through the production path. Before
that the latest iteration landed F11-C.2, the
catalog-wide deep-audit sweep: `mm2-inspect event --all` runs the
single-event closure check (records deep-parsed + validated,
`RaceDefinition`/`OpponentRoster` builds at both difficulties, wired
vehicle ids vs the catalog) on every authored row, one line each
plus per-event failure detail; `--city` restricts, `--strict`
aggregates the same conditions the single-event mode fails on plus
table errors and an empty catalog. Retail run-and-record
(`fnv1a64:e91e6cd4b2ae30d9`): 90/90 rows `ready`, 0 incomplete, 0
failed records/builds; strict exits 2 on 96 authored anomalies — the
disclosed 77 orphan `.opp` routes + sf `race0` 6-vs-7 mismatch, plus
18 per-record diagnostics attributed per row for the first time
(`AmbDenisty` misspells, omitted `Filename` labels, london
`exam1_1.csv`'s 4 short rows). `events`/`race-defs --strict` re-ran
green; `opponents --strict` exits 2 on the same 79 anomalies.
Evidence in docs/race-coverage.md. Before that the latest iteration
landed F07-B.9,
...
data, AUD-11), so `SurfaceVariant::for_weather` maps `rainy`
→ wet and every other selector → dry (designed, DSN-43;
the original's selection state is unrecovered, UNK-25),
`SurfaceAudio::load` probes `aud/cardata/player/<id>_surface
<variant>.csv` ahead of the shared `default_` (absent
candidate silent — stock ships none; malformed/wrong-kind
warns and falls through; a `default_` failure yields no
resource; never a cross-variant substitute), the bound
`variant`/`path` ride the resource, and the smoke record
gains ` surf=wet` on wet sessions only. The effective
weather resolves once through `effective_conditions` — the
same slot `env=ltNN`/fog consume. Retail headless sf:
`--weather 3` → `env=lt03(rainy-morning) surf=wet`;
`--weather 0` → `env=lt00(clear-morning)`, no marker.
Before that the latest iteration landed F07-B.7:
authored siren programs — `vpcop`'s horn-row `flags`
bit 4 toggles the city-keyed `<psdl-stem>policesiren.csv`
program (player) / shared `policesiren.csv` (opponent)
loaded into a `SirenAudio` session resource; each press
starts/stops a `SirenPlayback` that enters at sample 0,
picks one step per entry through the seeded `NavRng`,
dwells the authored `play time` on the session tick
(pause-freeze included) and follows `next index` between
samples — all retail programs cycle; malformed targets
end defensively under `MAX_SIREN_HOPS`. Voices are one
`PlaybackMode::Loop` child per current sample, respawned
on switch, non-spatial local / spatial remote (DSN-37),
bounded `MAX_SIRENS` 8, `SessionEntity`-swept; siren stems
prefer the `aud/*/sirens/` subtree (the exe's `sirens\%s`
scope), sentinel = authored silence, unflagged cars keep
the ordinary horn. Press-to-toggle on the horn control is
the designed reading — the original's trigger stays
unverified (DSN-42/UNK-25). `aud=` gains `/Nw/Ny`.
Retail `--car vpcop --horn --headless`: sf
`17w/1y` (the 4-sample chain walking), london `2w/1y`
(the 15 s ping-pong), vpbug shows no `w`/`y`.
Before that the latest iteration landed F07-B.6:
ambient-traffic engine voices off the authored
`*_engine.csv`/`default_engine.csv` tables —
`mm2_content::ambient_engine_audio` resolves the per-class
file then the authored default (designed binding; only
`va_bus_f`/`va_ddbus_l`/`va_diesels_s`/`va_smallsuv_s` match
a stock roster id exactly, so `va_sedan_s`/
`va_garbagetruck`/`va_pickup_f`'s tables are dead data under
it — AUD-9), `AmbientAudio` rides each spawned car,
`ambient_engine_rigs` builds one bounded spatial
`PlaybackMode::Loop` child voice (`MAX_AMBIENT_VOICES` 32,
`AmbientRig` marker so failures warn once and recycled
bodies re-attempt), and `ambient_engine_drive` re-mixes
pitch off `LinearVelocity` through the authored speed bands
— first covering band in file order wins (the `0–500`
catch-all would otherwise shadow the tight bands), uncovered
speeds read the nearest edge, `engine volume` constant
(DSN-41/UNK-25). `aud=` gains `/Ne/Nn` (spawned/audible).
Retail headless: london `--frames 1500` → `20e/16n`, sf →
`34e/16n` (`e` tracks `sp` spawns; `n` is the live fleet;
`0s` honest — no output device). Before that the latest
iteration repaired the F07-B.5 external review's doc
...
`SurfaceMaterial` resolves through the material's authored
`sound` class to a table row, the designed `tire_slippage`
quantity (or `|vel_long|` for speed-banded rows) picks the
covering skid band, and `|forward_speed|` mixes the authored
`max speed` rolling window — `PlaybackMode::Loop` voices
spawned lazily per committed entry, `SURFACE_DWELL`-debounced
on surface switches, bounded `MAX_SKID_VOICES`/`MAX_SURFACE_RIGS`,
spatial for non-local cars (DSN-37/DSN-39). F07-AC03's legs
hold by construction: a held brake at rest and airborne wheels
resolve nothing. Before that the latest iteration landed
F07-B.3: impact one-shot voices off the deduplicated
`ImpactEvent` stream through `default_impacts.csv` (DSN-38).
Before that the latest iteration landed F07-B.2: opponent
engine rigs + the spatial
mix — opponents spawn with `VehicleAudio` (opponent-side
cardata), `engine_rigs` builds rigs on every `VehicleAudio`
car bounded `MAX_ENGINE_RIGS` 16, non-player loops are
`PlaybackSettings::spatial` emitters (`ENGINE_SPATIAL_SCALE`
0.25 — rodio inverse-square + L/R pan), the player's rig
stays non-spatial, and `audio_listener` keeps one
`SpatialListener` on the active `Camera3d`; rig/listener
commands order `.after(session::drive_session)` after a
restart-frame dead-entity command race surfaced in the
smoke restart test. Retail `london circuit:0 --bot`:
`aud=0h/18v/0s/18l/17a/8r` — 8 rigs/18 loops, DSN-37.
Before that the latest iteration
landed F10-B.11: same-tick junction-entry arbitration — a
committed crossing now claims its junction for the rest of the
drive tick (`Junctions::entered`/`commit`/`release`, cleared by
`advance_tick`), closing the frame-start-snapshot race where two
eligible cars could take one box at once; synthetic test verified
to fail without it, retail legs show the deterministic signature
(london `jq=2→3 crx=68→66`, sf bit-identical). The same iteration
repaired the F13-C.6 review's doc findings (re-anchor aggregate
160→380/+138% corrected to 180→430/+139%; `opp_rec` glossed as
re-anchor teleports only — escapes are the separate `e` field;
the over-bound displacement anomaly now names all three legs:
london-3 110 m, london-5 58 m, london-9 30 m), commit `e72304f`.
The latest iteration repaired B.11's review finding: the
landing-rollback path called a junction-keyed `release` on a claim
the rolling-back car never held, so on a mixed-rule junction it
could strip another car's same-tick commit — the record is now
keyed by car (`BTreeSet<(u16, Entity)>`, `commit`/`release` take
the entity), the commit runs on `Entered` before the landing
check so a rollback genuinely releases its own claim, and a new
mixed-rule app test reproduces the harmful interleaving end to
end (verified `shared=2` under a junction-keyed release).
Before that the latest iteration
landed F13-C.6: the deep-course extended-budget probe — the 11
events whose parked-Pro field topped out at ≤3 gates re-run at
`--frames 24000` (~394 s, 2× budget), all `status=pass`; zero added
finishes, field re-anchors +139% vs +19% banked gates — the
deep-course non-finishes are predominantly controller-skill-bound
(the F15-B residual), not budget-bound; sf-6's leader banked all 6
gates and was racing to the armed finish trigger at cap. The same
iteration repaired the F13-C.5 review's doc findings (sf-2 hold
1/9→2/9; the "9 events" gate count → 10 with sf-8's `cp=1` at
`moved=37 m` disclosed as a likely opponent-shove crossing), commit
`feccdc4`. Before that the latest iteration
landed F13-C.5: the Professional hold-driver legs — 24 legs at
`--pro --frames 12000` with the blind `Hold` driver, all
`status=pass`, published in `docs/race-coverage.md` — the Pro matrix
is now complete at 72 legs = 24 events × scripted + parked + hold
(12 opponent finishes across 5 events under the blind driver, which
never resolved; london-10 rs25 wreck-loop matches amateur; london-4's
209-fall punt loop recurs verbatim). The same iteration repaired the
F13-C.4 review's doc findings (parked-leg finish count 16→13, the
london-2 comparison direction, the stale Amateur-parked sentence,
the raw-artifact location). Before that the latest iteration
landed F13-C.4: the complete Professional runtime matrix — 48 legs
(24 events × scripted + parked drivers) at `--pro --frames 12000`,
all `status=pass`, published in `docs/race-coverage.md` (13 opponent
finishes across 5 events under the parked control, 3 scripted local
finishes; london-4's 209-fall recovery loop disclosed). Before that
the latest iteration
landed F13-C.3, the sf-8 hypervelocity banger cascade bound
(`MAX_BANGER_LINEAR_SPEED`/`MAX_BANGER_ANGULAR_SPEED` stamped on every
banger body via `banger_bundle`, `angular_kick`/`activate_bangers`
write-side clamps, `MM2_SMOKE_DIAG` instrumentation; external review
pass — sf-8 re-verified `status=pass` in ~84 s). Before that
the latest iteration
landed F13-C.2: the `--parked` stationary-control driver (a
`ParkedDrive`-gated `parked_drive` system holds the handbrake through
the production `VehicleInput` path in both runners; the dev-world
drive check is inert for it) plus the first new matrix legs —
7 parked control runs on the C.1 finish events (**11 opponent
finishes across 5 events while the local participant never raced**;
sf-0's `cp=1/6` is an honest opponent-shove through a trigger) and 3
scripted `--pro` legs (sf-0 `finished place=6/7` behind 5 opponent
finishes, sf-3 `place=1`, london-0 `rs=2` still racing — Pro
measurably selects different authored rosters/conditions). Published
in `docs/race-coverage.md`. Before that the latest iteration landed
F13-C.1 (the checkpoint-catalog runtime matrix — 48 Amateur legs over
24 events × scripted/hold drivers plus the `result_outcome`
participant-identity fix; external review pass). Before that the
latest iteration
landed F02-C.5, closing the last F02-C legs: all 21 ready cars
render-captured + visually inspected on the retail install (Apple
M1/Metal), the per-car override causality sweep measured on every car
(engine ×0.5 → accel delta; grip ×0.5 → brake delta; saturated cells
recorded), the AC06 legs re-run (both cities headless, invalid-car
exit 2, mod override rendered), and the stale-screenshot false pass
the render leg exposed is fixed — `smoke_test` clears the target
before requesting the capture so a reused path can no longer pass on
the previous image. Before that the latest iteration
landed F15-B.11, the scripted-player re-anchor occupancy parity the
F15-B.10 review named as the one non-research gap: `scripted_drive`'s
`blocked` closure now runs the same [`reanchor_occupied`] leg —
extracted from `opponent_drive` so both recoveries share one
definition — against a frame-start snapshot of every `Player`-marked
participant (self included) plus same-frame `claimed` poses; the
penned-bot test parks a participant on the predicted claim and the
landing walks a further 8 m down the leg. Retail 9000f sf circuit:0
`--bot` keeps the envelope (`cp=4/9 lap=2/3 pos=1/5 opp=0/4
opp_rec=5 cu=4`, `p_rec=3r/6e`). Before that the latest iteration
landed F15-B.10, the representative avoidance matrix +
occupancy-aware re-anchoring: the re-anchor `blocked` closure now
keeps `REANCHOR_CLEAR` (8 m XZ, 4 m band) off every participant and
off poses claimed by same-frame re-anchors — two penned cars sharing
a projection can no longer teleport interpenetrating — and four
production-path tests cover the spec edge row (moving-blocker
overtake, overturned self-right, junction-miss rejoin, competing
re-anchors). Retail 9000f sf circuit:0 `--bot` keeps the F15-B.7/9
envelope (`cp=4/9 lap=2/3 opp_rec=5 cu=4`). Before that the latest
iteration
...
| F02-C | implemented | F02-B | Split: C.1 (roster coverage matrix + the reverse-band repair it caught — implemented below), C.2 (probe legs + the two trailer fixes they caught — implemented below), C.3 (`vpmoonrover` launch finding resolved — four-`vehWheel` architecture + back-back followers — implemented below), C.5 (per-car render leg + per-car override matrix + AC06 re-run + the stale-screenshot fix the leg caught — implemented below). Open disclosures kept in `docs/vehicle-coverage.md`: left/right + teleport verbs at synthetic level only, non-default paints not rendered, `vpcentury` non-monotonic brake leg flagged for the handling owner. |
...
| F02-C.5 | implemented | F02-C.3 | Per-car render leg + per-car override matrix + AC06 re-run — and the stale-screenshot false pass the leg caught: `smoke_test`'s capture-wait accepted any non-empty file at the target, so a reused `--screenshot` path reported `status=pass` on the previous image while the fresh save was still in flight (observed live: `pass` printed with stale bytes, then "Failed to send screenshot: sending on a closed channel" as the app tore down). `smoke::clear_stale_screenshot` removes the target before the capture is requested — `landed` can only see this run's write — and the run fails explicitly when the target cannot be cleared; regression test `smoke::tests::a_stale_capture_is_cleared_before_the_new_request`. Render leg (retail `fnv1a64:e91e6cd4b2ae30d9`, Apple M1/Metal, windowed, 2026-09-24): all 21 ready cars at one pinned waterfront spawn + fixed free cam (`--spawn=-141.9,1.5,-608.5,115 --cam=-144.9,7.0,-588.5,-8.5,-15 --frames 120 --screenshot`) — 21/21 `smoke=visual status=pass`, every PNG inspected: correct car at default paint on real SF geometry, both trailer rigs draw hitched trailers, `vpmoonrover` shows its authored nose-up stance, `vpcoop`/`vpcoop2k` and `vpbug`/`vpvwcup` pairs are distinct models not re-textures; captures local under `screenshots/f02c-render/` (gitignored). Override sweep: per car the dumped `VehicleConfig` TOML edited twice through the same `apply_handling_override` as `--vehicle-config` — `peak_torque_nm`+`max_power_w` halved (`engine ×0.5`) measured on the accel probe, every `longitudinal_grip` halved (`grip ×0.5`) measured on the `--controls` brake leg. Every car's measurement moves on both legs; saturated cells recorded, not argued away: `vpbullet`/`vppanoz` launches traction-limited (0-100 parity — `--trace` diverges mid-launch, 47.8→38.9 m/s at t=8 / 72.5→64.6 at t=14), seven cars' tops rev-limited not power-limited (<1% delta), `vpbus` n/r under the halved engine (−24% top), `vpmoonrover` top collapses 12.9→1.6 m/s (stop leg unsuitable — brakes from ~1 m/s), `vpcentury` stops *shorter* under halved grip (24.2→13.7 m; 0.75× lands at 11.2 — non-monotonic wheel-lock interplay, mechanism unverified, flagged for the handling owner). Baseline drift disclosed + corrected: the dynamic table's `vpcentury`/`vpmoonrover` stop cells predated C.3's 6→4 wheel rig (re-measured 2.3 s/24.2 m and 0.1 s/0.1 m; all other cells unchanged). AC06 re-run: `sf`+`london` headless `status=pass` 120 updates each, unknown `--car` exits 2 `unknown vehicle id "nonexistent"`, `checker-override` mod mounts at priority 300 + renders (magenta checkerboard capture). `docs/vehicle-coverage.md` gains the render + override sections. Candidate pending external check. |
...
| F06-C | queued | F06-B | — |
| F07-A | active | F01-B, F02-B, F06-A | Split: A.1 = bounded RIFF/WAVE decoder + `aud/cardata`/`aud/ambient` grammar parsers + `mm2-inspect audio` audit — all 3256 retail `aud/**` files classified, 2503 PCM waves decoded, 103 tables parsed, 0 failures/issues on retail (checked). A.2 = voice lifecycle + first runtime consumer — authored horn plays on ENTER through the bounded PCM path (`bevy_audio` enabled via custom `Decodable` asset), session-owned bounded voices, `--horn` dev trigger, `aud=` smoke field (checked at `cd4dd48`). Task text complete; row stays `active` because the feature's remaining consumers are F07-B scope — engine/impact/surface/siren/ambient application semantics (UNK-25), `spchdata`/`creaturedata` + DirectMusic (F08 scope). |
| F07-A.1 | implemented | F01-B | `mm2_formats::wav` — bounded RIFF/WAVE parser (chunk walk w/ even-byte padding, `fmt `/`data`, unknown-chunk order/offsets preserved, `riff_form_type` for non-WAVE RIFF); `mm2_formats::cardata` — 12 path-dispatched grammars: car audio (3 authored engine-column layouts preserved positionally), engineparams (binary MSVC-fill name prefixes kept raw), impacts (`***`/`""""` separators, `ENDOFDATA`), surfaces (`max speed`-window dry/wet vs 12-col divisor ice schemas — per variant both sides, tunnel index), sirens (`play time,next index` step chains), ambient engine/horn, object audio + `VECTORPOINTS`, ambient `file names` containers, semidata/vehtypes, suspension/tirewobble bands. `mm2-inspect audio <dir> [--strict]` — census, PCM census by rate/ch/bits, DirectMusic form classification (`DMSG`/`DMST`/`DLS `/`DMBD`), sample-ref→wave-stem cross-check (159/160 resolve; `skidflagstone` dead), cardata↔tune-roster coverage (`vpcaddie59` orphan), quirks vs issues split; `--strict` fails on failures+issues only. Retail: 0 failures, 0 issues, 35 authored quirks, 1 finding, 1 dead ref. Inventory audio-family note + `docs/research/audio.md` + AUD-1…8/UNK-25. No playback — parsed/decoded only. Checked. |
| F07-A.2 | implemented | F07-A.1 | Runtime voice layer + first consumer: `wav::lookup_stem` shared stem normalization; `VehicleDef.audio` side-loads `aud/cardata/{player,opponent}/<id>.csv` (warnings, never sinks the load); `mm2_game::audio::VehicleAudio` carries authored `CarAudio`; `mm2_app::audio` — `PcmAudio` `Decodable` asset through the bounded `wav` parser (non-PCM/oversized rejected pre-materialization), session `WaveBank` census with `aud22`-over-`aud11` tie-break, `AudioVoice`/`HornRequest`/`AudioReport`, ENTER fires one one-shot clip (`PlaybackMode::Despawn`, `MAX_VOICES` 8, pause-synced, session-teardown cleanup — DSN-35); `--horn` dev evidence trigger outside `record_eligibility`; `aud=<h>/<v>/<s>` smoke field. Retail: headless `aud=1h/1v/0s`, windowed `aud=1h/1v/1s` (real mixer attach). Gates: fmt/clippy/test green, 68 suites/1037 tests/0 failures; Cargo.lock unchanged. Candidate pending external check. |
| F07-B | active | F07-A | Split: B.1 = RPM-driven engine loop rig for the local player (candidate at `280edb9`, pending external check), B.2 = opponent rigs + spatial emitters + camera listener (candidate at `eec3f62`, pending external check), B.3 = impact one-shot voices off the deduplicated `ImpactEvent` stream (checked), B.4 = surface skid/rolling loop voices off wheel contacts (below), B.5 = clutch one-shot on committed `(gear, direction)` changes (checked at `2cc6e8d`), B.6 = ambient-traffic engine voices (checked at `27d284d`), B.7 = authored siren programs (below), B.8 = weather-bound surface-table variant (below). Remaining: scrape/continuous-impact semantics, AC02/AC05 scripted + audible evidence. |
| F07-B.1 | implemented | F07-A | Engine loop rig: `mm2_game::audio::EngineLoopSpec` resolves a cardata `Engine wave name` row's canonical fade-window columns (divisor schemas → `None`, never guessed) and `mix(rpm)` computes the designed envelope/pitch reading — fade-in `0→1` × fade-out `1→0` envelope interpolating `Min→Max Volume` (silent outside the band), `Min→Max Pitch` across the pitch window, degenerate windows as steps (DSN-36, inferred — UNK-25). `mm2_app::audio`: `EngineVoice`/`EngineRig` + `engine_rigs`/`engine_drive` — one `PlaybackMode::Loop` voice per resolvable+decodable row, parented to the player car (`ChildOf` cascade + `SessionEntity`), bounded `MAX_ENGINE_VOICES` 8, unresolvable/undrivable rows counted in `AudioReport.failed`, RPM re-mix each frame applied to the sink when attached (`set_volume`/`set_speed`), phase-ungated (countdown/results keep sounding; `Paused` sinks held by `sync_audio_pause`). Player-only, non-spatial (local car at the listener). `aud=` gains `<loops>l/<audible>a`. Tests: +6 mm2_game (retail-row resolve, idle band split, crossfade/pitch ramp, degenerate cut, divisor-schema rejection, non-finite safety), +6 mm2_app (rig spawn as car children, RPM re-mix, row/wave failure reporting + single-build, voice bound, no-audio car, teardown cascade). Retail (`fnv1a64:e91e6cd4b2ae30d9`): headless `--parked --frames 300` → `aud=0h/4v/0s/4l/3a` (vpbug's 4 authored loops, 3 audible at 900 rpm idle, honest no-sink), windowed `--frames 120` → `aud=0h/4v/4s/4l/3a` (all four attached real mixer sinks). Candidate pending external check. |
| F07-B.3 | implemented | F07-B.2 | Impact one-shot voices: `BangerDefinition.audio_id` kept verbatim from `BangerData` (0 on every retail record); `mm2_game::audio` — `impact_category` (struck-side `AudioId`→category `ID`, id-0 `WALL` fallback — designed binding, UNK-25) + `pick_impact` (`min,max force` band filter, `frequency`-weighted draw with all-zero→uniform fallback, `min,max volume` gain draw, non-finite-safe). `mm2_app::audio` — session `ImpactAudio` resource (player-side `default_impacts.csv` + generation-seeded `NavRng`; absent/malformed warns once, no resource) inserted/removed with the session; `impact_voices` system after `drive_session` in both schedules consumes the deduplicated `ImpactEvent` stream: one `PlaybackMode::Despawn` voice per *vehicle* participant (remote skipped — own client's authority; non-vehicle skipped), force = `severity × striker mass` (`ComputedMass`→`Mass`→config, each checked before fallback), spatial emitters at the impact point under `ENGINE_SPATIAL_SCALE` except the local player (non-spatial, DSN-37), bounded `MAX_IMPACT_VOICES` 12 with `+Nd` drops; `aud=` gains `<impacts>i` when nonzero. Tests: +4 mm2_game (category select/fallback, band select + sub-floor silence, frequency weighting + uniform fallback, volume range + non-finite bounds), +7 mm2_app (band pick + authored volume/spatial legs, prop `AudioId` category, two-car per-side impulse, remote/non-vehicle skips, 12-voice bound, stale generation, absent table, teardown sweep). Candidate pending external check. |
| F07-B.4 | implemented | F07-B.3 | Surface skid/rolling loop voices: `mm2_game::audio` — `SkidUnit` (the `skid wave` header's unit column: `slippage` vs `speed`, per-variant schema split verified on both sides — AUD-6 corrected), `RollingSpec::mix` (authored `min,max surface volume`/`pitch` interpolated over `0..max speed`, `None` for divisor-schema rows — never guessed), `SkidSpec::pick` (covering `min,max` band → `min,max skid volume` interpolation; below every band = authored silence), `SurfaceSpec::from_entry` (each half independently `None` on sentinel/missing/unclassifiable fields), `tire_slippage` (designed trigger quantity — `max(|traction_demand|, |slip_angle|/peak)` clamped 0..1; the tire model has no wheel-speed state — UNK-25). `mm2_content::SurfaceTables::sound_index` — material `sound` class → table row (`Authored(i)`→def `sound`, `Unspecified`→`_default`; designed binding, UNK-25). `mm2_app::audio` — session `SurfaceAudio` (player-side `default_surfacedry.csv`; absent/malformed → no resource) + `SurfaceRig`/`SurfaceVoice`/`surface_voices` after `drive_session` in both schedules: grounded wheels only, loudest covering skid band + loudest rolling loop per car, `PlaybackMode::Loop` child voices spawned lazily, idle at 0 (no attach/detach churn), `SURFACE_DWELL` 4-frame debounce on entry swaps, `MAX_SKID_VOICES` 4 bands / `MAX_SURFACE_RIGS` 16 cars, spatial for non-local cars (DSN-37), `SessionEntity`-swept. AC03 legs hold by construction — brake-at-rest demands nothing, airborne wheels have no contact. `aud=` gains `/<skids>k/<rolling>g` gauges when nonzero. Tests: +7 mm2_game (schema resolution, divisor-schema policy, band select + sub-band silence, gain/ramp interpolation, reverse-speed, non-finite, utilization), +7 mm2_app (band pick + gain, AC03 rest/airborne legs, rolling mix + spatial split, dwell-gated switch, unresolvable/absent silence, rig bound, teardown). Retail (london `circuit:0 --bot --headless --frames 3000`): `aud=0h/70v/0s/18l/17a/8r/12i/8k/1g+125d` — surface rigs live on authored data, honest `0s` headless. Candidate pending external check. |
| F07-B.6 | implemented | F07-B.5 | Ambient-traffic engine voices: `mm2_game::audio` — `AmbientEngineSpec::from_table` (sentinel → `None` authored silence; non-finite band cells dropped; table with no usable bands still resolves at constant pitch) + `mix(speed)` — first covering band in authored order wins (retail's tight bands precede a `0–500` catch-all, so authored order is the only reading under which they're reachable), uncovered speed → nearest band edge (ties prefer the earlier row), `|velocity|` input, non-finite speed silent, `engine volume` constant (designed reading, DSN-41/UNK-25). `mm2_content::ambient_engine_audio` — `aud/cardata/ambient/<id>_engine.csv` then `default_engine.csv` (designed binding; `Ok(None)` = authored silence, `Err` = malformed resolved file, never a silent substitution; AUD-9 records only 4/7 per-class files match a stock roster id — `va_sedan_s`/`va_garbagetruck`/`va_pickup_f` are dead data under exact-match). `mm2_app::traffic` — `AmbientClass.audio` + `AmbientAudio` stamped at spawn. `mm2_app::audio` — `ambient_engine_rigs`/`ambient_engine_drive` chained `.after(session::drive_session)` in both schedules: one `PlaybackMode::Loop` `AudioVoice{AmbientEngine}` spatial child per car (`ENGINE_SPATIAL_SCALE`), `MAX_AMBIENT_VOICES` 32 bound (`AmbientRig` marker = one warn per car, recycled bodies re-attempt), `SessionEntity`-swept; drive re-mixes off parent `LinearVelocity` — lane followers and knocked wrecks publish the same component. `aud=` gains `/<n>e/<n>n` spawned/audible when nonzero. Tests: +4 mm2_game (resolve, authored-order/catch-all + nearest-edge + negative/non-finite, sentinel/bad-cell degradation), +8 mm2_app (per-class→default→absent resolution, malformed-errors, sentinel, spatial-loop spawn + report, speed-band mix legs, unresolvable-once, 32-voice bound, teardown). Retail (`fnv1a64:e91e6cd4b2ae30d9`, headless `--frames 1500`): london `aud=0h/52v/0s/4l/4a/1r/12i/8c/1k/0g/20e/16n+17d`, sf `aud=0h/61v/0s/4l/4a/1r/7i/8c/34e/16n+15d` — `e` tracks `sp` (every spawned car voiced, zero `+x` failures), `n` = the 16-car live fleet, honest `0s`. Candidate pending external check. |
| F07-B.7 | implemented | F07-B.6 | Authored siren programs: `mm2_game::audio` — `SIREN_FLAG` (horn-row `flags` bit 4; vpcop alone on retail — AUD-10 census: vpbus 1, vpcentury 2, vpddbus/vpsemi 8), `SirenSpec::from_program` (explosion binding carried, no consumer; authored volume sanitized), `SirenPlayback` — sample-0 entry, seeded-`NavRng` step pick per entry, `play time` dwell on the session tick, `next index` jumps between *samples* (all retail programs cycle; negative/out-of-range/empty/non-finite → `End`, `MAX_SIREN_HOPS` 16 bounds zero-time chains, overshoot banks into the next dwell) — all designed readings under DSN-42/UNK-25. `mm2_app::audio` — `SirenAudio` session resource (player `<psdl-stem>policesiren.csv` city-keyed + shared `opponent/policesiren.csv`; absent/malformed → no side, never fabricated), `WaveBank::load_siren` preferring the `aud/*/sirens/` subtree, `siren_toggle` (press-to-toggle on `HornRequest` for flagged cars — designed; unflagged cars keep the horn, `MAX_SIRENS` 8) + `siren_drive` chained `.after(session::drive_session)` in both schedules — one `PlaybackMode::Loop` `AudioVoice{Siren}` per current sample, respawned on `Switch`, non-spatial local / spatial remote (DSN-37), `SessionEntity`-swept, sentinel = authored silence, missing stem warns once per activation while the program walks. `aud=` gains `/<n>w/<n>y` when nonzero. Tests: +6 mm2_game (verbatim retail fixtures — sf 4-sample chain, london ping-pong, opponent closed-cycle soak — plus malformed/edge legs), +12 mm2_app (load-by-city/role, toggle on/off/re-arm, tick-driven switch respawns the voice, frozen-tick hold, missing/sentinel wave silence, flagged-without-program `failed`, malformed ≠ substitute, spatial split, bound, teardown, unflagged horn). Retail: sf `--car vpcop --horn --frames 1500` → `aud=1h/…/17w/1y` (17 voices across the authored chain in ~25 s), london → `…/2w/1y` (one authored 15 s switch), vpbug `--horn` → no `w`/`y` (ordinary horn). Candidate pending external check. |
| F07-B.8 | implemented | F07-B.7 | Weather-bound surface-table variant: exe strings carry `%s_surfacedry`/`%s_surfacewet` + `default_surface{dry,wet}` and **no** `surfaceice` — the shipped ice tables are dead authored data under every recovered binding (AUD-11). `mm2_game::audio::SurfaceVariant::{Dry,Wet}` + `for_weather` — `rainy` (3) → wet, all other selectors → dry (designed, DSN-43; the original's selection state unrecovered, UNK-25). `mm2_app::audio::SurfaceAudio::load(vfs, weather, vehicle)` probes `aud/cardata/player/<id>_surface<variant>.csv` then `default_surface<variant>.csv` (the `%s_` format the exe carries; catalog-id stem inferred — absent candidates probe silently since stock ships none; malformed/wrong-kind warns and falls through; `default_` failure warns → no resource; never a cross-variant substitute); the bound `variant`/`path` ride the resource. `mm2_app::session` resolves `effective_conditions` once for the env block *and* the surface load (same slot as `env=ltNN`/fog). `mm2_app::smoke` gains ` surf=wet` on wet binds only — dry/absent records bit-identical. Tests: +1 mm2_game (selector census), +5 mm2_app (rainy→wet end-to-end on authored samples, dry selectors, per-vehicle precedence, malformed fall-through, dry-only tree under rainy → none). Retail (`fnv1a64:e91e6cd4b2ae30d9`, sf headless `--frames 60`): `--weather 3` → `env=lt03(rainy-morning) surf=wet`, `--weather 0` → `env=lt00(clear-morning)` no marker, no probe warns. Candidate pending external check. |
| F07-C | queued | F07-B | — |
...
| F10-B.11 | implemented | F10-B.10 | Same-tick junction-entry arbitration (the same-tick half of the "queue-through-intersection priority" remainder). `blockers`/`bound_for` are frame-start snapshots, so a second eligible car evaluated *after* another car's commit in the same drive tick read the box empty and took the interior alongside it — the window is real wherever two approaches' gates open together (a green member's parallel lanes; a signal flow meeting an FCFS head whose dwell elapsed). `mm2_game::traffic`: `Junctions` gains a per-tick `entered: BTreeSet<(u16, Entity)>` — claims keyed by the committing car after the B.11 review found a junction-keyed release could strip another car's same-tick commit; `gate` reads `occupied = box_occupied \|\| entered(ix)` on exactly the two paths a gated rule can open (green member, FCFS head) — `NeverStop`/unruled ends keep authored free flow; `commit(ix, car)` claims the junction, `release(ix, car)` undoes only the caller's own rolled-back transfer, `advance_tick` clears the set once the next tick's physical box-yield sees the car. `mm2_app::traffic`: `drive_ambient` commits on `LaneAdvance::Entered` and releases its own claim if the landing-occupancy check rejects the transfer (revert cursor, zero speed, decrement `crossings`), then departs the FCFS slot as before. Tests: +2 mm2_game (`a_committed_entry_holds_the_box_for_the_rest_of_the_tick` — commit closes the now-front FCFS car, a free-flow car's rollback over a live claim sheds only its own, `advance_tick` sheds the record; `a_committed_entry_closes_a_green_approach_and_not_free_flow` — a commit closes a green approach mid-phase while NeverStop never consults it) +2 mm2_app (`a_same_tick_second_arrival_waits_for_the_committed_crossing` — two cars on parallel lanes of one member road reach the lane end the same tick; fixture gained a 2-lane variant; verified to fail without the record — `shared=2`; `a_rolled_back_free_flow_entry_keeps_the_committed_claim` — mixed-rule junction: a `NeverStop` car re-armed at its lane end rolls back over the gated car's same-tick commit every drive tick while a second gated car on a parallel lane arrives behind it; verified `shared=2` under a junction-keyed release). Retail (install `fnv1a64:e91e6cd4b2ae30d9`, `--headless --frames 3000`): sf `traf=16/16 sp=43 rec=27 dead=0 uns=0 q=0 jq=3 stuck=0 crx=59 jmp=0 kn=1 sig=647 sigd=3` bit-identical to B.9; london `traf=16/16 sp=22 rec=6 dead=0 uns=0 q=0 jq=3 stuck=0 crx=66 jmp=0 sig=828` vs B.9's `jq=2 crx=68` — the deterministic signature of a same-tick pair now serializing (one more held-approach observation, two fewer in-window commits). Open: same-tick claims are a designed approximation — a car already rolling when the gate closes stands at the lane end mid-green (policy-consistent, stricter than the original's unverified behaviour); box occupancy is still point-based; original junction arbitration unverified (UNK-12). Gates: fmt/clippy(locked)/test(locked) pass. Candidate pending external check. |
| F10-B.12 | implemented | F10-B.11 | Momentum-correct collision handover — a repair of B.6's free kick, the "without injecting extreme energy" half of the handover requirement. B.6's `knock_ambient` added a bounded kick on top of the solver's infinite-mass wall answer: the striker took a wall response AND the wreck took a free approach-speed launch — the same double-energy defect class F04-C.4's banger transfer fixed. `mm2_app::contracts`: the banger transfer math is now shared — `Transfer`/`resolve_transfer` (`(1+e)·v·μ` impulse, launch = J/m_struck), the `StruckMut` query tuple, `angular_share` (contact-lever Δω), `striker_correction`/`write_striker_correction`; `banger.rs` consumes them unchanged (24/24 banger tests). `mm2_app::traffic`: `knock_ambient` decides first (a `Knock` record per qualifying edge — deepest contact, push dir from the manifold normal on either collider side, launch, impulse, both levers, transfer) then applies: `Lane`→`Knocked` on the same entity (re-check dedups multi-edge hits), mass-correct launch + contact-lever spin, `RigidBody::Dynamic` insert, junction depart, `traffic.knocked` counter. The striker correction is a velocity target, not a returned impulse — instrumentation showed Avian's recorded `total_impulse` accumulating penetration-recovery and restitution passes (13641/22929 recorded vs ~9800/19500 actual Δv·m), so the striker's `dir` component is rewritten to `struck_pre + severity − J/m_s` directly — conserving by construction. A lane-follower striker takes no correction (`drive_ambient` owns its velocity), and a wreck striker this pass already flipped is skipped — a follower-follower edge never charges the exchange twice; unresolved masses keep the approach-speed launch with no correction. Same authority/phase gate and drain as `drive_ambient` — no predicted handover, no stale burst after pause. Tests: `a_hard_hit_hands_the_follower_to_dynamics` extended (wreck slowed to its share range instead of dead-stopped, striker rewritten to its share instead of the ~15 m/s wall match, no-injection momentum bound, exactly one flip across 60 re-contacting ticks) + new `a_light_striker_shares_the_exchange_not_its_speed` (400 kg block → parked 1200 kg car: both leave at the ~6 m/s inelastic common velocity, momentum conserved — the exact-share leg); fixture followers now carry production `Mass`/`CollisionEventsEnabled`. Retail headless (install `fnv1a64:e91e6cd4b2ae30d9`): sf `--frames 3000` → `traf=16/16 sp=41 rec=25 dead=0 uns=0 q=0 jq=2 stuck=0 crx=56 jmp=0 kn=4 sig=647 sigd=3`, london `--frames 1200 --spawn 0.4,5.5,-720,0` → `… crx=33 kn=2` — real handovers on both cities through the production path, all counters finite; london `--frames 3000` plain → kn=0 (the Hold driver grounds out on props at 95 m — no handover exercised, honestly recorded). Open: single-point impulse pair, not per-contact impulses; original ambient crash behaviour unverified (UNK-12); no rendered/manual evidence. Candidate pending external check. |
| F10-B.13 | implemented | F10-B.12 | Wreck spin bound + striker-class disclosure — repairs the B.12 external review's named verification gaps and covers AC03's damage leg. `spawn_ambient_car` stamps `MaxLinearSpeed(MAX_BANGER_LINEAR_SPEED)`/`MaxAngularSpeed(MAX_BANGER_ANGULAR_SPEED)` — the solver-side bounds every banger body gets, inert while the car is kinematic (`drive_ambient` owns its ~15 m/s lane velocity, far under the cap) and binding once an impact flips it dynamic; `knock_ambient`'s contact-lever spin write now clamps at `MAX_BANGER_ANGULAR_SPEED` — the review's gap: a transient spike could leave a wreck spinning fast enough to inflate a later contact's `normal_speed` reading, the same cross-contact amplifier the sf-8 cascade rode (the striker-correction path keeps B.12's parity with the banger write — its share pre-exists unclamped there too). Each handover also counts its striker's class into `knocked_by_participant`/`_ambient`/`_other` (`Player` marker, `AmbientCar`, or anything else — banger bodies, break fragments, world-side bodies) → the record's new `kns=Np/Na/Nx` field beside `kn=` — the review could not tell whether a participant ever struck a car; sf's `2p` now says two did. Tests +2 (`tests/traffic.rs` 31 → 33): `a_participant_striker_takes_damage_and_names_the_class` — the session's *real* player vehicle (stamped with authored-style `VehicleDamage` bounds) slides into a parked follower: the car flips (`knocked_by_participant == 1`, `Dynamic`, solver bounds present), an `ImpactEvent` emits and the striker's damage accrues `severity × follower mass` through `collect_impacts → apply_impact_damage` (now wired into the fixture app in production order) — AC03's damage leg proved end to end; `a_wreck_striker_counts_as_ambient` — a sliding dynamic wreck flips a queued follower and counts `a`; `a_light_striker_shares_the_exchange_not_its_speed` asserts `x` on the plain block. Fixture followers gained the production `Mass`/bounds shape. Retail headless (`fnv1a64:e91e6cd4b2ae30d9`): sf `--frames 3000` → `kn=4 kns=2p/2a/0x` (two participant strikes, `dmg=23a/0d/0r`), london `--frames 1200 --spawn 0.4,5.5,-720,0` → `kn=2 kns=0p/0a/2x` — both `x`, neither a participant nor an ambient car. Candidate pending external check. |
| F10-B.14 | implemented | F10-B.13 | Striker-correction spin bound + same-tick pileup coverage — repairs the B.13 review's named gaps. The shared `write_striker_correction` (banger `apply_striker_correction` + both ambient striker paths) clamps the post-write angular velocity at `MAX_BANGER_ANGULAR_SPEED` — a `Player` striker carries no solver bound, so the write clamp is its only one; linear writes untouched. `a_same_tick_pileup_flips_the_car_once` (`tests/traffic.rs` 33→34): two strikers contacting one lane car on the same update (proven through the `Collisions` graph; the `CollisionStart` drain lags one fixed step for both) → `knocked==1`, one `x` charge, single-transfer wreck launch (~5–9 m/s not ~2×), winner/loser split (corrected ~6 share vs uncorrected ~14 wall shove, `max−min>4`), no re-flip across 60 ticks. `contracts.rs` unit tests +2 (10⁶ rad/s share writes 60.0 in-direction, under-bound verbatim + pre-existing-spin total clamp). The B.13 review's Avian question is resolved from dependency source: `clamp_velocities` iterates every `SolverBody` incl. `IS_KINEMATIC`-flagged — non-binding, not inert, on lane followers (comments corrected). Fixture wreck in `a_wreck_striker_counts_as_ambient` now carries production solver bounds. Retail headless (`fnv1a64:e91e6cd4b2ae30d9`): sf `--frames 3000` → `kn=4 kns=2p/2a/0x dmg=23a/0d/0r`, london `--frames 1200 --spawn 0.4,5.5,-720,0` → `kn=2 kns=0p/0a/2x` — both bit-identical to B.13 (bound inert at ordinary speeds). Candidate pending external check. |
| F10-B.15 | implemented | F10-B.14 | Multi-edge collision accounting repair + the symmetric-edge coverage the B.14 review disclosed untested. The defect: striker corrections were per-edge *velocity targets* — a striker contacting two cars in one drain had its second target write erase the first edge's payment (both cars launched on one transfer — momentum injection), and a striker the same pass itself flipped was skipped outright (a same-tick daisy chain's last car launched uncharged). `knock_ambient`'s apply is now two passes — flips first (each `Knock` captures `struck_pre`, the struck car's pre-launch velocity along `dir`), then striker corrections compounding per striker: the first committed edge writes the wall-returning velocity target, every later edge is a pure `−dir·J` debit. `handed_over` keys on `(car, its edge's striker)`: a car flipped on a *different* pair pays the pure debit (the kinematic–kinematic edge charged it no wall — Avian 0.7 broad phase creates k-k pairs for moved proxies; only the solver skips them), while the follower-follower mutual pair still owes nothing — its struck-side launch already is its share. Tests +4 (`tests/traffic.rs` 34→38): `one_striker_pays_both_cars_it_flips` — a wide 2600 kg block contacting two parked followers the same update (`Collisions`-proven): `knocked==2`, `kns x=2`, striker ≈7 m/s (paid both, vs ~14 for one target write), both wrecks launched; `a_same_tick_chain_charges_the_middle_car` — a wide driving follower's front reaches a striker block and a light parked neighbour the same step; the neighbour's 200 kg mass puts the mutual `B←C` orientation under the impulse floor so B's flip edge can never alias the pair it strikes: `kns x=1 a=1`, B debited past its own launch (~0.9 vs ~2.7 uncorrected), C launched, the block target-corrected; `a_mutual_follower_edge_charges_the_exchange_once` — a driving follower clipping a parked neighbour flips both (k-k edge, solver-unsolved), `kns a=2`, the exchange splits with no second charge on the mover; `a_same_tick_three_striker_pileup_flips_the_car_once` — B.14's edge extended to three strikers: `knocked==1`, one `x` charge, one corrected striker + two wall-shove losers (`sorted[1,2] − sorted[0] > 4`). Fixture gains `spawn_shaped_follower` (caller-chosen hull width/mass) and `two_lane_install` is reused for parallel-lane staging. Retail headless (`fnv1a64:e91e6cd4b2ae30d9`): sf `--frames 3000` → `kn=4 kns=2p/2a/0x dmg=23a/0d/0r`, london `--frames 1200 --spawn 0.4,5.5,-720,0` → `kn=2 kns=0p/0a/2x` — both bit-identical to B.14. Candidate pending external check. |
| F10-B | active | F10-A | B.1 obstruction response + B.2 authored junction rules + B.3 bounded stuck recovery + B.4 spawn occupied-space rejection + B.5 junction-box yield + B.6 collision handover + B.7 authored signal indicators + B.8 union of player interest areas + B.9 junction-interior crossing paths + B.10 startup WARN aggregation + B.11 same-tick junction-entry arbitration + B.12 momentum-correct handover + B.13 wreck bound + striker-class disclosure + B.14 striker-correction spin bound + same-tick pileup coverage implemented above. Remaining: collision fidelity vs AC03's full checklist (player-hit feel — manual; the damage leg now has synthetic + retail-counter evidence), original junction/spawn timing and interior-crossing geometry (UNK-12), signal-prop model fidelity (real unit geometry — none ships; axis convention unverified), F10-AC evidence vs the spec. |
| F10-C | queued | F10-B | — |
...
| F11-C | active | F11-B | Split: C.1 (single-event dependency inspection — implemented below), C.2 (catalog-wide deep-audit sweep `event --all` + the retail run-and-record leg — implemented below; AC01's listed-with-status leg and AC06's inspect leg now have catalog-wide retail evidence). Remaining: AC02–AC05 promotion stays at the landed runtime slices' test evidence (swept-trigger/teleport/reset cases in `tests/race.rs`, countdown/restart lifecycle in `tests/session.rs`/`tests/event.rs`, once-only ledger results in `tests/results.rs`) — no new runtime work this slice. |
...
| F11-C.2 | implemented | F11-C.1 | Catalog-wide deep audit — `mm2-inspect event <dir> --all [--city <stem>] [--strict]` sweeps every cataloged row through the same `inspect_event` closure check the single-event mode runs (records deep-parsed + validated, `RaceDefinition`/`OpponentRoster` builds at both difficulties, wired vehicle ids vs the catalog). `--city` is now optional (`--all` restricts it; single-event still requires it) and `--event` conflicts with `--all`; the vehicle catalog scans once per run instead of once per row. Output: table statuses, one line per event (`ready`/`incomplete`, record count, `defs ok/ok`, `rosters ok+Ni`), indented per-event failure detail, per-city summary with the extras count. Strict = a missing/malformed table, an empty catalog or any per-event `failures()` — same conditions as the single-event mode, aggregated. Tests +3 (`mm2_inspect` 27 → 30): sweep reports every row in order incl. incomplete ones, a record failure surfaces under its event, an empty catalog fails. Retail run-and-record (`fnv1a64:e91e6cd4b2ae30d9`, this commit): `event --all` → 90/90 events `ready`, 0 incomplete, 0 failed records, 0 failed builds; `--strict` exits 2 on **96 authored anomalies** — the same 77 orphan `.opp` routes + `sf/race0` 6-vs-7 mismatch the `opponents` audit reports, plus 18 per-record diagnostics the catalog audits count but don't attribute per row (8 `AmbDenisty` misspells, 6 omitted `Filename` labels on london `crash8` + sf `crash4`/`crash9` data pairs, 4 short rows skipped in london's `exam1_1.csv` — a crash3 waypoint file). Sibling legs re-run: `events --strict` rc 0, `race-defs --strict` rc 0 (64 defs/city), `opponents --strict` rc 2 (79 authored anomalies). Evidence published in docs/race-coverage.md. Candidate pending external check. |
...
| F13-C | active | F13-B, F15-B | Split into C.1 (catalog × runtime × opponents matrix — implemented below), C.2 (stationary-control driver + first Professional legs — implemented below), C.3 (sf-8 hypervelocity banger cascade diagnosed + bounded — implemented below) C.4 (complete Professional scripted+parked matrix — implemented below), C.5 (Professional hold-driver legs — implemented below; the Pro matrix is now complete at 72 legs = 24 events × 3 drivers) and C.6 (deep-course extended-budget probe — implemented below; stalls predominantly controller-skill-bound, not budget-bound). Remaining: original-fidelity comparisons (difficulty, pacing, AI competence vs retail are unverified — engine self-metrics only). |
| F13-C.1 | implemented | F13-B, F15-B | Checkpoint-catalog validation on the retail install (`fnv1a64:e91e6cd4b2ae30d9`, commit `1791df0`, 2026-09-24, Apple M1/Metal). Denominator kept honest: 24/24 authored Checkpoint rows (12/city, all `ready`, 0 failed race-def builds), 26 Crash Course rows held out as unsupported (F21), 79 authored audit anomalies disclosed not filtered (77 orphan `.opp` records, `sf/race0` `6opp/7tbl` mismatch, `stunt0` dead route ref). Runtime matrix: 48 legs = 24 events × scripted/`Hold` drivers at `--frames 12000`, Amateur; 48 `status=pass` — the sf-8 scripted leg passed but needed ~24 min wall (CPU-bound under load, `dropped=53955`, one transient 17 km/s velocity spike; pose finite). Opponents start and progress on every uninterrupted generation; 18 opponent finishes with ledger results across 7 events (london-0/2, sf-0/2/3/4/5); 3 local finishes (london-0 place 3, sf-3/sf-4 place 1). Restart contamination measured, not hidden: rs up to 25 (hold london-10) with `dup=0` — the restart machinery itself passes a hard soak; scripted london-6/8 + hold sf-8 end in countdown restart loops (driver wrecks under authored damage rules). Code fix: `smoke::result_outcome` could surface the field *leader's* `outcome=`/`place=` when the local participant was still racing (seen live on sf-0: `cp=2/6` + `outcome=finished place=1`); now resolves by local identity or omits the field. Published: `docs/race-coverage.md`. AC01/AC04/AC06 legs landed, AC02/AC03/AC05 mapped; fidelity and the residuals above stay open. Candidate pending external check. |
| F13-C.2 | implemented | F13-B, F15-B | Stationary-control driver + first Professional legs (retail `fnv1a64:e91e6cd4b2ae30d9`, commit `a7d2797`, 2026-09-24, Apple M1). `Driver::Parked` + `--parked` (conflicts `--bot`): `input::ParkedDrive` gates `parked_drive`, which writes `VehicleInput{handbrake:1}` on the player vehicle every frame — the foot brake is the reverse throttle once stopped, so the handbrake is the parked state; same resource-gated pattern as `ScriptedDrive` in windowed + headless (chained after it so a test holding both markers stays deterministic). The dev-world `car never drove` verdict is inert under Parked — it never requests motion, so `moved=`/`peak=` are the evidence; record names `driver=parked`. Tests: +1 `dev_world_parked_driver_stays_parked` (pass + `driver=parked` + peak<5 + moved<5). Retail legs at `--frames 12000` (`docs/race-coverage.md`): **7/7 parked control passes** on the C.1 finish events — local `cp=0` on 6 (sf-0 `cp=1/6`/`moved=8m` is an honest opponent-shove through a trigger — a pushed crossing is a real crossing), `pos=` last everywhere, `dup=0`, **11 opponent finishes with ledger results across 5 events while the local participant contributed nothing** (london-0 3/4, sf-0 4/6, sf-2 1/6, sf-3 2/5 — masked by the scripted leg's local win — sf-4 1/5); london-2/sf-5 progressed without finishing (omax 4/5). **3 scripted `--pro` legs**: sf-0 `finished place=6/7` behind 5 opponent finishes (pro `vpcaddie`/`vpbug` field, lt01 — wired `6opp 6rt` clean; the `6opp/7tbl` mismatch is amateur-only), sf-3 `finished place=1` (`vpvwcup`/`vpbullet`/`vpauditt`, lt06), london-0 `rs=2` racing at cap (`vpcoop2k` ×6) — Pro selects measurably different authored rosters/conditions. Sharpens AC03/AC04; Pro matrix partial (3/24), fidelity open. Candidate pending external check. |
| F13-C.3 | implemented | F13-C.1 | sf-8 scripted-leg defect root-caused and bounded (retail `fnv1a64:e91e6cd4b2ae30d9`, Apple M1; reproduced `status=fail — fell through the world`, ~9 min for 1200 frames, ~19 GB `MALLOC_SMALL` churn, Avian `store_contact_impulses`/`warm_start` dominating). Instrumented diagnosis (`MM2_SMOKE_DIAG`, kept as env-gated smoke instrumentation): fragment `angular_kick` ÷ tiny cuboid inertia → extreme ω → next contact's `normal_speed` reads ω×r surface velocity as approach `severity` → `resolve_transfer` launches the next generation's pieces at ~that magnitude (158 → 10⁵ → ~4×10⁶ m/s across ~10 frames) → hypervelocity fragments tunnel city-wide, breaking 3915 props and flooding 10-25k collision events/frame; fragment blowback produced the recorded `peak=17070 m/s` car spike. Fix (implementation choice — records carry no speed limit): `mm2_game::banger` `MAX_BANGER_LINEAR_SPEED=200`/`MAX_BANGER_ANGULAR_SPEED=60` (legit transfer launches cap ~(1+e)·striker ≈ 180 m/s); `banger_bundle` stamps Avian `MaxLinearSpeed`/`MaxAngularSpeed` on every banger body (substep clamp + writeback); `angular_kick` clamps its output; `activate_bangers` clamps `severity` at `Activation` construction on both contact and bound-strike paths and clamps the written `launch` — components never hold absurd values between frames (constraint prep reads them pre-clamp). Tests +3 `tests/banger.rs` (21→24): bundle carries caps, spawned 5000 m/s/10⁶ rad/s body clamps within a frame, hypervelocity striker's struck-prop launch stays bounded. Retail re-run: sf-8 `--bot --frames 12000` in **~70 s** (was ~24 min) — `status=pass`, `peak=30.1`, `dropped=0`, `bng=5941d/13a/4s/3b`, race progressing `cp=4/8 pos=1/7`; zero bodies >150 m/s. Unbounded remainder disclosed: vehicles carry no cap (spike was fragment blowback, bounded at source); transient single-substep post-solve spikes re-clamp next substep but can still be read between frames — now clamped at the gameplay read sites. Candidate pending external check. |
| F13-C.4 | implemented | F13-C.3 | Complete Professional runtime matrix (retail `fnv1a64:e91e6cd4b2ae30d9`, code commit `11af1ea`, 2026-09-24, Apple M1) — 48 legs = 24 events × scripted (`--bot`) + parked (`--parked`) at `--pro --frames 12000`, **48/48 `status=pass` rc 0**. Authored `.aimap_p` rosters + Pro parameter blocks drive every event (4–7 opponents each). Scripted local finishes: london-0 `place=4` (3 opponents finished ahead), london-2 `place=1`, sf-3 `place=1`. Parked control: **13 opponent finishes with ledger results across 5 events with the local participant contributing nothing** (london-0 5/6, london-2 2/7, sf-0 4/6, sf-2 1/7 `vpford` 9/9 gates, sf-3 1/6 — 16 counting both driver legs, london-0's scripted leg adding 3 while the local raced to place 4); `cp=0` on 22/24 parked legs (sf-0/sf-8 `cp=1` = the same disclosed opponent-shove trigger crossings seen at amateur); `peak` ≤10.3 m/s, `moved` ≤29 m = contact displacement. Restart contamination matches the amateur pattern: scripted wreck-loops london-1 rs5 / london-3 rs8 / london-6 rs5 / sf-10 rs6 (still `Countdown` at cap); parked legs on the same events show the field progressing (omax 2–7). Anomalies disclosed: london-4 parked `rcv=209f/209r` — the stationary car on elevated spawn ground is repeatedly punted over the edge by field contact and recovered to its anchor every time (`status=pass`, `wheels=4/4`, `moved=5m`); modest `dropped` ≤345 on 5 legs (vs amateur's 16k–56k — the F13-C.3 caps hold under Pro); peak ≤34.2 m/s, no spikes. Divergence from C.2's three Pro legs (run at a7d2797, pre-caps): sf-0 scripted `place=6/7` → `cp=2/6` at cap after one wreck-restart; london-0 scripted at-cap `rs=2` → `place=4` finish; sf-3 `place=1` unchanged — the C.2 rows are marked superseded, not deleted. Published: `docs/race-coverage.md` C.4 section. Sharpens AC01 (full Pro catalog driven)/AC04 (control leg at both difficulties)/AC06; Pro hold legs, deep-course budgets and original-fidelity comparison remain open. Candidate pending external check. |
| F13-C.5 | implemented | F13-C.4 | Professional hold-driver legs completing the Pro matrix (retail `fnv1a64:e91e6cd4b2ae30d9`, code commit `7fb6cf6` — docs-only over `11af1ea`, identical code, stamped in every leg log; 2026-09-24, Apple M1) — 24 legs = 24 events × the blind `Hold` driver at `--pro --frames 12000`, **24/24 `status=pass` rc 0**; Pro coverage is now 72 legs = 24 events × scripted + parked + hold. `Hold` drives blind at full throttle — a bad field participant, not a control: it never resolved (no `place`), banked ≥1 gate on 10 events before wrecking (cp ≤2 — sf-8's `cp=1` at `moved=37 m` reads like the parked legs' disclosed opponent-shove crossings rather than driven progress; the other nine moved 105–589 m), and holds the matrix's worst restart counts — london-10 rs25 (matching amateur hold's 25 on the same event), london-3 rs17, london-9 rs9 (still `Countdown` at cap — the amateur sf-8 rs13 / Pro scripted sf-10 rs6 class), london-8 rs8, sf-1 rs9, sf-11 rs6, london-2 rs5; every one is a driver outcome — the same events' parked legs show the field progressing (omax 2–7). **12 opponent finishes with ledger results across 5 events while the hold car raced and never resolved** (london-0 4/6, sf-0 4/6, sf-2 1/7, sf-3 2/6, sf-4 1/5) vs the amateur hold legs' 12 across 7. london-4 repeats its parked-leg anomaly verbatim: `rcv=0w/209f/209r` on the elevated spawn, `wheels=4/4` — the loop is the spawn ground's, not the driver's. Record honesty: `dup=0` wherever the results/damage pipeline ran; london-3/london-9 omit the whole `dmg=`…`txl=` tail because `impacts=0` (activity gating keeps idle generations bit-identical; `results=0` — nothing to duplicate). `wheels=0/4` london-9/london-11, `3/4` london-2 — disclosed end poses. `dropped` ≤21, `peak` ≤48.6 m/s (blind full throttle downhill — bounded, no spikes). Raw legs local at `/tmp/mm2-pro-hold/` (per the large-capture rule). Published: `docs/race-coverage.md` C.5 section. Closes the Pro matrix on AC01/AC06; deep-course budgets and original-fidelity comparison remain open. Candidate pending external check. |
| F13-C.6 | implemented | F13-C.5 | Deep-course extended-budget probe (retail `fnv1a64:e91e6cd4b2ae30d9`, code commit `feccdc4` — docs-only over `7fb6cf6`, identical code, stamped in every leg log; 2026-09-24, Apple M1). The matrices' residual named budget as a possible cause of the deep-course non-finishes: the 11 events whose parked-Pro leader topped out at ≤3 gates at 197 s (london-3/4/5/6/9/10, sf-6/7/9/10/11) re-ran at `--parked --pro --frames 24000` (~394 s sim, 2× budget) — **11/11 `status=pass` rc 0**. Result: **predominantly controller-skill-bound, not budget-bound** — zero added finishes (`opp=0`, `results=0` everywhere), field re-anchors 180 → 430 (+139%) against summed banked gates 121 → 144 (+19%); the leader advanced on 6 events (london-4 2→5/6 largest) and plateaued on 5 (london-5/9, sf-9/10/11). sf-6's `vpauditt` banked all 6 checkpoints — finish trigger armed (RACE-7) — and was still racing to it at cap: the closest a deep-course opponent has come to resolving. Parked-control anomalies scale with budget unchanged in kind: london-4's punt loop 209 → 461 falls, plus the same spawn-edge fall loop on london-5 (122 → 348f), sf-9 (43 → 172f), sf-11 (49f+7w → 304f+7w); london-3's parked car took `cp=1/6` at `moved=110 m` (the disclosed opponent-shove mechanism, driven further) with `rcv=17w` water recoveries + `dead=10` ambient in the spawn-adjacent pileup. `dropped` ≤218, `peak` ≤12.8 m/s. Raw legs local at `/tmp/mm2-pro-deep/`. Published: `docs/race-coverage.md` C.6 section. Closes the budget question on the named residual; opponent driving competence (F15-B) and original-fidelity comparison remain open. Candidate pending external check. |
| F14-A | implemented | F02-B, F11-B | Split into A.1 (binding honesty + lap evidence — implemented below), A.2 (AC06 representative-playability matrix + the densify gate-coverage repair its analysis surfaced — implemented below), A.3 (route-bound Ordered AI progress over authored lines that miss gates — implemented below), A.4 (Professional scripted+parked matrix — implemented below), A.5 (Professional hold-driver legs — implemented below) and A.6 (AC05 circuit-restart evidence leg + AC promotion record — implemented below). London circuit0–11, SF circuit0–11 authored data present (circuit11 partial: opp/pathset only, no .aimap). SF `cir1–9` are the circuit events' start grids under a short stem — aliased to `circuit<N>` since F11-B.2 (WPT-3). All six ACs now carry evidence: AC01/AC02 externally checked at `561b8b7`, AC03/AC04 sharpened by the 100-leg both-difficulty matrix (two Professional opponent finishes), AC05's Ordered leg landed in A.6, AC06's matrices + audits published in docs/race-coverage.md. Remaining outside this task's scope: event `.aimap` `[Exceptions]`/density scoping needs consumers (F15/F10 scope); traversal-bound stalls under the new progress model (london-2 @gate0-bind ~700 m, london-5 @gate2-bind ~1050 m, sf-7 pack @gate5-bind ~985 m — fields never drive far enough to earn the bind, F15-B controller class); original-fidelity comparisons stay unverified (engine self-metrics only — no completability claim). |
...
| F14-A.2 | implemented | F14-A.1 | Complete-catalog Circuit runtime matrix + `densify_route` gate-coverage repair. Denominator: 20 authored Circuit rows (10/city, all `ready`; `circuit10/11` extras kept visible). 40 legs — every event × {`--bot`, `--parked`} at Amateur, `--frames 12000`, retail `fnv1a64:e91e6cd4b2ae30d9` — all `rc=0 status=pass`, logs local `/tmp/mm2-circuit-matrix{,-v2}/`, published in `docs/race-coverage.md` (Circuit section). The analysis leg found uniform field-wide plateaus at fixed gate indices (7 of 20 events pre-fix). Root cause #1 — **fixed here**: `NavGraph::densify_route` re-paths could abandon a checkpoint the authored `.opp` segment crossed (`london circuit:6` gate 0: authored crosses at 4.7 m inside r10 on a flyover whose banger-dressed ramp has no routable BAI lanes; the lane detour missed by 198 m → every Ordered participant stalled at cp 0). `densify_route` now takes the race's `Checkpoint` list and keeps the authored leg whenever the re-path would drop authored gate coverage (re-paths that still cover — or newly cover — stand); `driving_route(route, nav, gates)` threads `RaceDefinition.checkpoints` to both consumers (opponent roster + scripted-bot route); ledger DSN-44. tests/nav.rs +2 (rejected re-path keeps verbatim; covered re-path still densifies). Post-fix the london-6 driven line covers all 14 gates ≤4.7 m and every opponent went `0c → 1c` (plateau moved to gate 1); london-4's field deepened `7c → 9c` onto its authored-miss gate, sf-2's scripted leg `3/13 → 9/13`; london-9/sf-9 keep their plateaus past restored coverage — traversal residual (F15-B class), disclosed. Root cause #2 — **disclosed, not repaired**: four events' *authored* lines themselves miss gates (london-2 g0 11m/r7, london-4 g0 16m/r11+g9 32m, london-5 g2 36m, sf-7 g5-7 34-120m, uniform over every `-a-*`/`-p-*` route) — original AI progress must be route-derived, not trigger-bound (UNK-11 inference); binding AI Ordered progress to driven-route position is the next candidate task. london-2 parked control additionally logs `rcv=241w` (stalled field punts it into the Thames — collateral, kept visible). `status=pass` ≠ completable: published as evidence with the stalls named, not claimed as F14 closure. Candidate pending external check. |
| F14-A.3 | implemented | F14-A.2 | Route-bound Ordered AI progress — the A.2-disclosed second defect (authored `.opp` lines missing gate cylinders) now has a designed model (DSN-45; original accounting stays UNK-11). `mm2_game::race` gains `RouteGateLine`: at bind, every checkpoint projects to the driven polyline's closest-approach arc, re-based to the spawn arc and clamped non-decreasing in authored order (the Ordered sequence is always earnable by driving the line — a gate's trigger crossing cannot precede its bind position because the bind *is* the line's closest approach). `measure` projects the pose onto the chased leg (absolute arc + lateral); `wrap()` counts closed-route traversals at each chase-index wrap; `reanchor()` resyncs traversals at the stuck-recovery landing so a walk-back across the route boundary can't bank undriven arc. `RaceProgress::advance_route` credits the next required Ordered gate once `arc_high` passes its bound (closed routes offset `lap × loop_len`; open routes bind the first traversal only — no retail lapped event ships one, disclosed bound). `route_clears` counts route-derived clears separately from physical `crossings`; `advance` still wins wherever the car really crosses (no double-count). `mm2_app`: `spawn_opponents` binds a line per Ordered roster entry with a resolved route (player and `AnyOrder` never carry one); `opponent_drive` grows `arc_high` only while the pose projects within `ROUTE_ARC_LATERAL` 25 m of the chased leg — a car punted onto a parallel road earns nothing; `advance_race` applies `advance_route` to bound participants; smoke `opps=` rows suffix `/Nd` on activity. Tests: tests/race.rs +7 (negative pre-wrap measure, authored-line-miss credit, no double-count, closed-route lap wrap, AnyOrder non-binding, open-route first-traversal bound, re-anchor resync) → 35; tests/opponents.rs +2 production-path (route-less binds nothing; authored-miss gate earns progress through `load_session_world`/roster/`opponent_drive`/`advance_race`). Retail (`fnv1a64:e91e6cd4b2ae30d9`): v3 rerun of the four authored-miss events × {bot,parked} — 8/8 `rc=0 status=pass`; london-4 parked **two opponents complete lap 0** (`0c/2l`, `/2d` each) and sf-7 one (`5c/2l` `/10d`) — the missed gates bank by route arc where cars reach the binds; london-2/london-5 hold `0c`/`2c` with `0d` — honest traversal residuals (fields never reach the ~700 m/~1050 m binds under permanent pile-up churn), not progress-model failures; `results=0` everywhere. Published `docs/race-coverage.md` v3 section. Candidate pending external check. |
| F14-A.4 | implemented | F14-A.3 | Professional Circuit matrix — the representative-difficulty leg the A.2/3 evidence named open (F13-C.4 pattern). 40 legs = 20 cataloged events × {scripted `--bot`, parked control} at `--pro --frames 12000`, retail `fnv1a64:e91e6cd4b2ae30d9`, commit `e43ee20` (docs-only delta on `4a1db5a`; every leg log stamps it), logs `/tmp/mm2-circuit-matrix-pro/` — **40/40 `rc=0 status=pass`, `dup=0` everywhere**. Pro measurably selects `.aimap_p` rosters and authored lap counts (london `*/4` vs amateur `*/3` on circuit0/3–8, `*/2` on c1/c2/c9; sf `*/4` except c8/c9 `*/2`). First catalog-wide run with DSN-45 live: `/Nd` activity on 14/20 events, `0d` exactly where fields never reach the first missed bind (london-2/5, sf-7 — traversal residuals, unchanged). Multi-lap churn is real: opponents complete ≥1 lap on 10/20 events — london-0's whole 7-car field laps (16/18 completions, four cars reached lap 4, one `vpcoop2k` finished `6c/4l/F` → `results=1`, local `phase=playing` — remote resolution ends nothing), sf-0 all-four scripted, sf-1 5–6/7 lap-0 completions banked `8–10d`/10 gates each (route-credit extreme — the driven path threads almost no cylinders), sf-5 one car two laps. Scripted driver never finishes at Pro (best london-0 `lap3/4`) — Pro measurably harder. Parked control honest (`cp=0` all 20; `moved` ≤53 m displacement). Anomalies: london-8 `58f` both legs, sf-5 parked `222f` spawn-edge loop, london-6 Thames punt on both legs (`243w`/`239w`), sf-9 only `dropped` (105/112) + `peak` 35.1. Published `docs/race-coverage.md` Pro section. Sharpens AC01/AC03/AC04/AC06 evidence; hold-driver legs remain open. Candidate pending external check. |
| F14-A.5 | implemented | F14-A.4 | Professional hold-driver legs completing the Pro matrix (docs + evidence only — no code change; retail `fnv1a64:e91e6cd4b2ae30d9`, binary stamped `commit=6ea22d7` — code-identical to `86bae3e`). 20 legs = 20 cataloged events × the blind `Hold` driver (settles ≤2 s, full throttle, no steering — a bad participant, not a control) at `--pro --frames 12000`, logs `/tmp/mm2-circuit-matrix-pro-hold/` (~27 min wall, ≤102 s/leg) — **20/20 `rc=0 status=pass`, `dup=0`**; the Professional Circuit matrix is complete at 60 legs = 20 events × scripted + parked + hold. London-0 produced the matrix's second Pro finish — `vpcoop2k` slot 2 `6c/4l/F`, `results=1`, `phase=playing`, a different winner than the scripted leg's slot 0 (once-only ledger + local-races-on hold under the third driver); opponents lapped on 7/20 legs (vs 10 under scripted/parked). Driver-independent classes confirmed: london-8 `58f` identical on all three drivers, sf-4 56f/58f/58f; sf-5's loop scales with driver (132f/222f/100f). Blind-driver hazards: london-2 `rcv=328w/5f` (worst Thames loop of the matrix), london-4 `dropped=493` (worst Circuit leg to date), sf-8 `peak=64.8 m/s`. Traversal residuals unchanged (`0d` at london-2/5, sf-7 pack under every driver). Published `docs/race-coverage.md` hold section. Candidate pending external check. |
| F14-A.6 | implemented | F14-A.5 | AC05's Ordered restart leg + the AC promotion record (the last named open slice — the driver-leg matrix closed in A.5). `tests/opponents.rs` gains `restart_restores_the_circuit_grid_counters_and_objects`: a synthetic `mmcircuitdata` event (NumLaps 2, 2-opponent roster, authored `cir0` grid, closed `.opp` loops) loads through `load_session_world` → `advance_race` like every event test; the field banks real progress mid-race (gates/lap/route credit, race `Running`), then `SessionControl.restart` rides the production teardown/reload and generation 2 asserts every AC05 clause: race resource re-minted (`generation=2`, `Countdown`, `clock=0`), the two-car lineup respawned exactly once (`SessionEntity(2)`), opponents back on authored grid slots `index+1` with `RaceProgress` zeroed (`lap/next/cleared/crossings/route_clears` all 0, `AwaitingStart`), `OpponentDriver.next` restored to its spawn-time chase index per roster index, each `RouteGateLine`'s `arc_high` equal to a fresh `bind` at the respawned pose (route credit un-banked), the player back on authored slot 0, one marker per Ordered gate (no finish trigger on circuits), and `standings_in(2)` empty — the retained ledger can never rank a stale result into the new race. Prior AC05 legs stood on AnyOrder events only (`restart_respawns_the_lineup`, `restart_rebuilds_the_event_session`, `restarting_the_event_respawns_its_overlay_once`, results-row restart). AC promotion recorded on the parent row and in `docs/race-coverage.md`: AC01/AC02 checked at `561b8b7`, AC03 multi-lap/multi-participant independence + ordering (tests + matrix churn), AC04 real circuits raced against opponents at both difficulties (two Pro finishes), AC05 this leg, AC06 100-leg matrix + catalog audits published. Candidate pending external check. |
| F14-B | implemented | F14-A | B.1 (live running order — implemented at iteration of `live_order`/`pos=`) + B.2 (Ordered lap-validation edge coverage — implemented below). All four named items carry evidence: lap validation (`Ordered` swept-sequence contract + the B.2 edge legs), participant ranking (B.1's `live_order` + F13-B.1's `standings`), HUD instruments (HUD-2's Circuit set — lap record `lap x/y`, checkpoint count, place, stopwatch — in `update_hud`), opponent hooks (Ordered roster spawn/drive + DSN-45 route-bound progress, retail 60-leg Pro matrix). Remaining scope is F14-C's: catalog validation, multi-lap/opponent/restart and exploit-negative legs on retail content (also deps F15-B). Original-fidelity of the Ordered model stays unverified (UNK-11). Candidate pending external check. |
| F14-B.2 | implemented | F14-A | Ordered lap-validation edge coverage — the spec's edge list exercised through the production `advance_race`/`reanchor_teleported_participants` path in `tests/race.rs` (+7: 36→43). New `ordered_def` helper builds the retail shape (course gates in authored order + lifted start-line copy last). Legs: `ordered_skipped_gate_clears_nothing_until_revisited_in_order` (sweeping gate 1 while gate 0 is owed banks nothing — not even a `crossings` tick — and the skipped gate must be re-visited), `ordered_finish_line_is_inert_until_it_is_next` (repeated both-direction line sweeps before its turn bank no lap), `ordered_spawn_inside_the_line_grants_nothing` (staged dead-centre on the closing gate — dwell + movement inside the volume banks nothing), `ordered_overlapping_closing_gate_banks_one_lap_once` (one segment through an overlapping last-gate/line pair banks the lap exactly once; post-finish re-sweeps mint nothing), `ordered_last_lap_tie_records_both_deterministically` (shared-clock finishes both record, standings break by `PlayerId`), `ordered_reset_over_the_line_still_owes_the_crossing` (the production `ResetVehicle` jump sweeping the closing gate banks nothing; the line must be re-crossed), `an_unresolved_participant_does_not_block_the_local_result` (a never-resolving opponent keeps the race `Running` but the local finish still reaches `Results` with the bounded ledger). No production code changed — the `Ordered` contract already enforced every case; the slice adds the evidence and promotes the parent. Candidate pending external check. |
| F14-C | active | F14-B, F15-B | Split into C.1 (mid-race restart + authored-edge retail legs — implemented below). Remaining: catalog-wide validation of completability claims stays gated on F15-B's research/controller work (traversal stalls on london-2/5/6/9 + sf-7/9); original-fidelity of the Ordered lap model stays UNK-11; no *driven* local circuit finish exists (the `--finish` leg is a dev-swept resolution). |
| F14-C.1 | implemented | F14-B | Mid-race restart + authored-edge Circuit evidence (retail `fnv1a64:e91e6cd4b2ae30d9`, Apple M1, headless). `DevOverrides::restart_at` + `--restart-at <ticks>` (120 Hz session-clock units) + `dev_restart_at` — the tick-gated form of `--restart`, scheduled beside `dev_restart_once` ahead of `drive_session` in both Update chains; `record_eligibility` gains the `restart-at` arm. Tests: +3 `tests/smoke.rs` (deferred firing — gen-2 `ticks=` remainder proves it; one-shot across generations; never-reached threshold never fires) + the eligibility arm in `tests/progression.rs`. Retail legs: sf `circuit:0 --bot --restart-at 7200 --frames 12000` → `rs=1`, gen-2 re-racing `cp=2/9 lap=2/3 pos=1/5`, `dup=0` (control leg at the same wall point showed `cp=4/9` banked); london `circuit:0 --bot --restart-at 7200` → `rs=1`, gen-2 `cp=1/6 lap=3/3`, `vpcoop` slot 1 finished `6c/3l/F` minting `results=1` scoped to generation 2; sf `circuit:0 --parked --spawn=<row-0 line>` → the car dwells inside the authored closing-gate cylinder all 3600 frames and banks `cp=0/9 lap=1/3 results=0` while the field laps; sf `circuit:0 --finish --frames 4000` → `phase=results`, `cp=9/9 lap=3/3 place=1`, `results=1` — the full 9-gate × 3-lap Ordered sequence resolved through production `advance_race`, opponents unresolved (`opp=0/4`). Sharpens AC02 (authored-geometry edge leg), AC05 (mid-race restart with banked progress — both cities), AC03/AC04 (generation-scoped opponent finish post-restart). `--finish`/`--restart-at`/`--spawn` legs are record-ineligible evidence-runner runs — classified, not stock-fidelity claims. Candidate pending external check. |
...
| F15-B.10 | implemented | F15-B.8 | The spec's edge-case matrix + the real gap it exposed. Audit first: persistent obstruction (`blocked_route_drives_around_the_parked_car`, `authored_avoid_*`) and shortcut invalidation (`Teleported` segment-break tests) were already covered; three edges were not, and "competing recovery positions" needed code — `reanchor_pose`'s `blocked` checked only un-cleared gates, so two cars whose stuck windows expire on the same frame (a shared pen, a pileup) teleported onto the same projected spot. The closure now also rejects candidates within `REANCHOR_CLEAR` 8 m XZ / `REANCHOR_CLEAR_Y` 4 m of any participant (self included — a car beached on the line does not teleport onto itself) or of a `claimed` pose another re-anchor took this frame (the `traffic` snapshot predates the loop); the occupancy walk consumes the same `REANCHOR_WALK` 60 m budget — disclosed bound, jammed-solid still lands. Positional, not avoidance — applies whatever the authored avoid flags say. Tests +4 (`tests/opponents.rs` 36→40), all through `load_session_world`/`opponent_drive`: `a_faster_car_passes_a_slower_moving_blocker` (same-lane roster slot staged ahead at authored maxThrottle 0.15 vs 1.0 — position order swaps on track, pass leaves the lane line, bus never sustained-shoved, still `Racing` when the follower finishes), `an_overturned_opponent_self_rights_and_resumes` (roof-down flip → authored `self_right_delay` flops it upright in place, `reanchors == 0`, then a real finish), `a_junction_miss_rejoins_the_route` (teleport past a corner → bounded rejoin — the `point_reached` plane rule may mark the missed anchor or the chase bends back — finish earned through post-leg gates), `competing_reanchors_land_clear_of_each_other` (two penned cars, identical projections: landings upright, on the line, ≥ `REANCHOR_CLEAR` apart, zero gates banked). Retail (`fnv1a64:e91e6cd4b2ae30d9`): `sf circuit:0 --bot --headless --frames 9000` → `status=pass cp=4/9 lap=2/3 pos=1/5 opp=0/4 opp_rec=5 cu=4` — the B.7/9 envelope holds. Newly named gap: `scripted.rs`'s player re-anchor keeps gate + ground-probe rejection only — no participant occupancy (evidence-driver scope). Candidate pending external check. |
| F15-B.11 | implemented | F15-B.10 | Scripted-player re-anchor occupancy parity — the F15-B.10 external review's named follow-up: `scripted_drive`'s `blocked` closure kept gate + ground-probe rejection only, so a penned `--bot` car could teleport onto a participant and contaminate the evidence it exists to produce. The occupancy leg is now shared: `opponents::reanchor_occupied` (extracted verbatim from `opponent_drive`, so the two recoveries cannot drift) rejects a candidate within `REANCHOR_CLEAR` 8 m XZ / `REANCHOR_CLEAR_Y` 4 m of any position in the frame-start snapshot `scripted_drive` now takes of every `Player`-marked participant — the scripted car itself included — plus a `claimed` vec for same-frame claims (a session fields one `PlayerVehicle`, but the structure matches the opponent loop). Test +1 (`tests/bot.rs` 18→19): `penned_bot_reanchor_lands_clear_of_a_parked_participant` pens the bot off its line, waits for `Racing`, reproduces the walk-back with the gate leg alone to find the pose it would claim ((−116,−140) on the test route), parks a `Player`+`Position` participant there, and asserts the real landing is on the line, upright, ≥ `REANCHOR_CLEAR` off the blocker, and banks nothing — verified non-vacuous (the leg disabled lands the car 0.004 m from the blocker). Gates: fmt/clippy/workspace tests clean. Retail (`fnv1a64:e91e6cd4b2ae30d9`): `sf circuit:0 --bot --headless --frames 9000` → `status=pass cp=4/9 lap=2/3 pos=1/5 opp=0/4 opp_rec=5 cu=4 p_rec=3r/6e` — the B.7/9/10 envelope holds. Candidate pending external check. |
| F15-C | queued | F15-B | — |
...
| F20-B | queued | F20-A | — |
| F20-C | queued | F20-B | — |
| F21-A | active | F02-B, F11-B, F16-B | London crash0–12/exam/final/reverse180 (cab course) and SF crash0–12 + accel/collide/corner/evade/frogger/jump/stunt/exam (stunt course) present. A.1 (the independently audited lesson catalog — implemented below) covers the req-1 audit leg: stages, sub-events, difficulty tables, aimap wiring, overrides, rewards. Still open: prerequisites/start-condition semantics, environment props runtime binding, the `Event` code's evaluator dispatch and tail-column semantics (UNK-35), instruction/voice/subtitle linkage, and per-lesson objective evaluators — structural catalog ≠ playable lessons. |
| F21-A.1 | implemented | F02-B, F11-B, F16-B | Crash Course lesson catalog audit (F21 req 1's audit leg). `mm2_content::crashcourse` (DSN-55): `CourseCatalog::scan` views the shared `EventCatalog`'s CrashCourse events as lessons — `LessonStage` decoded from the authored `Description` tags (the CC-2 order), both `mmcrashdata` param blocks verbatim, `data.csv`/`data_p.csv` split into Amateur/Professional `LessonTable`s (inferred `_p` mapping — tighter limits/higher density on retail, sf `crash9` pro even links `reverse180_p.csv`), each row a `LessonSubEvent` carrying the raw `Event` code + inferred `LessonObjective` decode (0 jump/2 follow/3 cop-chase/4 corner/5 cross-traffic/7 maneuver/8 stop/9 map — measured correlation, unknown codes stay `Unknown(n)`, CC-7/UNK-35), `Filename` waypoint links resolved through the VFS, own-stem aimap per difficulty via `event_aimap` distilled to police counts/vehicle ids + case-insensitive `.opp` route resolution + `[CopChaseDistance]`/`[Exceptions]`/`[Ambient Types]` counts, `<object>_crash<N>` extras re-attributed per lesson (inferred suffix convention; unclaimed extras counted not filtered), `crash,N` rewards attached. `mm2_formats::crashdata` now retains the authored header cells — the only in-file tail-column evidence (`AmbDenisty` typo, drifting names preserved). `mm2-inspect crash-course <install> [--city] [--strict]` audits both cities: strict fails on empty catalog/incomplete events/unresolved links/missing or empty difficulty tables/aimap errors/dead `.opp` wires/vehicle ids outside `VehicleCatalog`. Tests +9: formats +1 (columns retention), content +5 (stage parse, full objective enum incl. 1/6-unknown, complete lesson view, incomplete+unresolved reporting, empty-catalog denominator), inspect +3 (clean course, unknown-vehicle wire, missing-table city). Retail audit (`fnv1a64:e91e6cd4b2ae30d9`, `docs/research/crashcourse.md`): both cities 13/13 ready, 0 unresolved links, 0 dead wires, rewards at crash3/7/11/12 matching CC-6; london 26 + sf 23 unclaimed extras surfaced. New findings: sf `crash6` (stop) wires a scripted `vpford` opponent (matching its `numopp`=1 tail); sf `crash1`/`crash2`/`crash4`/`crash12` share an identical ten-road `[Exceptions]` block; london `exam1_2`=7 vs sf `exam1_2`=8 — the code belongs to the row not the filename; professional `mmcrashdata` rows author different tod/weather than amateur on several lessons. Iteration-73 review repair: the research doc's falsified measurements corrected (the ten-road `[Exceptions]` block is shared identically by sf `crash1`/`crash2`/`crash4`/`crash12`, not `crash12` alone; `tail[1]`=1 on both `crash4` lessons; `stop`/`exam1_2` rows carry `tail[2]`/`numopp`), and `CrashDataFile::diagnostics` now surface on `LessonTable` + audit `note:` lines so a dropped row can't silently shrink the denominator. Candidate pending external check. |
| F21-B | queued | F21-A | — |
| F21-C | queued | F21-B | — |
| F22-A | active | F01-A, F02-B, F11-B | Split into A.1 (authored in-race HUD minimap — implemented), A.2 (the `I` opponent indicator — implemented below), A.3 (the `H` HUD master toggle — implemented below), A.4 (the authored stopwatch/countdown timer — implemented below), A.5 (the authored `hudarrow*` compass arrow — implemented below) and A.6 (the remaining HUD-2 instruments — checkpoint list/lap record/place indicator — implemented below). F22-A stays open: F22-AC01–AC06 acceptance evidence (multi-leg retail runs, `H`/`I`/restart interactions on original data) is partial, and every instrument's original on-screen presentation is unrecovered (UNK-30..34) — all layouts are designed readings. Dev HUD (speed/gear/RPM/grounded/cam pose) exists. |
...
| F22-A.2 | implemented | F22-A.1 | Documented `I` opponent indicator (HUD-3/CTL-1). The documentation records only the toggle — the original's indicator presentation is unrecovered — so the instrument's look is a designed reading (DSN-51, UNK-30): the authored `geometry/hudmap_tri.pkg` bound through the VFS (`absent:<why>` on the report, never a substitute), stood upright apex-down over each opponent's authored collider/`chassis_size` roof, scaled to a designed in-world extent, yaw-facing the active `WorldCamera3d` camera, painted from the shared `TRI_PAINT_OPPONENTS` authored palette so an opponent's arrow matches its map tri (both pools bind in entity order). Landed: `mm2_app::oppind` — `OpponentIndicators(bool)` session-agnostic toggle (on by designed default), `OppIndReport` (`markers`/`bound`/`absent`), `spawn_opponent_indicators` (event sessions only, pool sized to the authored roster, all `SessionEntity`-stamped), `indicator_input` (`I` in `Playing`/`Countdown` only — overlays keep the key, same contract `mirror_input` holds for BACKSPACE), `drive_opponent_indicators` (rebinds the pool to live non-local `Player` participants per frame — despawn/vehicle-drop frees the slot same update, `Remote` control binds like `Ai`, the local car never carries one; runs ungated by `capturing` for `--frames` captures). `hudmap.rs` helpers promoted `pub(crate)` for the shared authored content (`read_pkg`/`authored_extent`/`paint_material`/`TRI_PAINT_OPPONENTS`). Smoke records ` ind=<on|off>/<pool>m/<bound>b` or `absent:<why>` on event sessions only — cruise/dev-world records stay bit-identical. Evidence: `tests/oppind.rs` +5 (phase gating incl. Menu/Paused/Results key ownership, authored binding + per-car roof heights + local exclusion + Remote + despawn/vehicle-drop release + hidden-while-off demand, missing-pkg report, synthetic-event `headless_smoke` leg `ind=on/1m/1b`, dev-world field absence); retail sf `checkpoint:0` headless `ind=on/6m/6b` with `pos=3/7`; windowed countdown capture shows painted arrows over the staged field. Candidate pending external check. |
| F22-A.3 | implemented | F22-A.2 | Documented `H` HUD toggle (HUD-3/CTL-1 "Toggle HUD (Heads-Up Display)"). Exe evidence (mm2hook `mmHUD`/`mmHudMap` layouts): the original's HUD is a single `mmHUD` node owning `mmHudMap` (which itself draws the in-world opponent indicators via `DrawIndicator`), `mmArrow`, the stopwatch/countdown `mmTimer` pair, `mmDashView` and `mmCRHUD`, exposing `Enable`/`Disable`/`Toggle` — so `H` is the master gate over the whole driving-HUD layer, not just the dev telemetry line. Landed: `mm2_app::hud` (new module — `HudVisible` session-agnostic resource on by designed default, `hud_input` toggling `H` only in `Playing`/`Countdown` so overlays keep the key, `update_hud` moved off the `mm2` bin target so the visibility gate is test-reachable). Every layer member now ANDs the gate onto its own visibility write while keeping its authoritative state: the instrument line (`ErrorText` is a load-failure surface, never gated), `update_nav_arrow`, `update_countdown_banner`, `update_race_warning`, `drive_hud_map` (corner/large views park — the *full-screen pause map* is a menu surface and stays up, `cam.is_active = map.visible() && (hud || fullscreen)`), `drive_opponent_indicators` (markers hide but `bound` still counts live demand), `sync_dash_visibility` (the cockpit cluster hides — the cockpit *camera* itself keeps rendering the windshield view, and the `MirrorCamera` child stays independently owned). `DevOverrides::no_hud` + `--no-hud` starts the layer off for frozen-input captures — render-only like `--cam`, deliberately out of `record_eligibility` — and the smoke record reports ` hud=off` on-activity only, so every earlier record stays bit-identical. Designed outside the layer (DSN-52): the mirror strip, nav-debug overlay, session/menu overlays; the exact original flag scope and whether the mirror suppresses stay unrecovered (UNK-31). Evidence: `tests/hud.rs` +8 (live-phase gating incl. Menu/Paused/Results key ownership, instrument-line hide/restore + `ErrorText` exemption, all three race instruments across the countdown-digit and low-time legs, map camera parks with pause-map exemption, indicator markers hide while `bound` still counts, cockpit cluster hides under `CameraMode::Cockpit`, session-agnostic survival through teardown, `hud=off` record leg via the real `headless_smoke` pipeline + default-record field absence). Candidate pending external check. |
| F22-A.4 | implemented | F22-A.3 | HUD-2's stopwatch/countdown timer pair (the `mmTimer`s the recovered `mmHUD` layout owns). Authored-side verified: the install ships `digitac_0..9` (41×56 alpha-keyed green digits), the `digitac_*_half` 20×27 set, `digi_colon`/`digi_colon_half` separators, and `race_{lap,chk,opp,rec,chg,rnm,veh}` label tiles the still-open list/lap/place instruments will want. Landed: `mm2_app::racetime` — `RaceTimer` root (session-owned row, 9 slots `[m3 m2 m1 : s1 s2 : c1 c2]`, `UiTransform` percent-centred so the variable-width row stays on centre), `TimerDigits` glyph bank bound through `city::load_image` (all 22 stems; any miss → `absent:missing-glyphs`, no half-bound row, never substitute art), `RaceTimerReport` (`glyphs`/`absent`/`display`), `update_race_timer` driving slot `Display`/`ImageNode` off authoritative `RaceState` — `time_remaining` while a timed definition runs, `clock` otherwise; armed through `Countdown` (full limit / `0:00:00`), dark on `Complete` or a stale generation, `H`-gated while `display` keeps composing. Presentation is a designed reading (DSN-53, UNK-32): `m:ss:hh`, leading-zero suppression capped `999:59:99`, top-centre at 96 px under the nav arrow on a translucent plate tinted to the colon tile's own authored background; `LOW TIME` moved below the plate. Repairs folded in: `HudNodes` gained `RaceTimer` — plus `NavArrow`/`LowTimeWarning`, which without a `UiTargetCamera` fell back to `DefaultUiCamera`'s max-order primary-window pick (the mirror strip, order 2) and rendered inside it whenever armed. Smoke: `tmr=<glyphs>g/<m:ss:hh|off>` or `absent:<why>` on event sessions only — cruise/dev-world records bit-identical; `update_race_timer` runs ungated by `capturing` for `--frames` captures. Evidence: `tests/racetime.rs` +13 (slot composition incl. cap, full/partial/missing binding, untimed count-up `0:06:25`, timed count-down `0:40:00`, countdown arming, `Complete`/stale/cruise release, `H` gate, synthetic-event `headless_smoke` `tmr=22g/…`, absent leg, dev-world field absence); retail sf `checkpoint:0` headless `tmr=22g/0:00:33` matching `ticks=40`; retail windowed captures — checkpoint `0:04:44` agreeing with telemetry `4.4s`, blitz countdown armed at `0:30:00` beside the `1` banner, cockpit-retargeted `0:02:73`, mirror strip parked over the nav-arrow band with the timer just under it (documented overlap). Candidate pending external check. |
| F22-A.5 | implemented | F22-A.4 | RACE-6 compass arrow bound to the authored `hudarrow*` packages (HUD-2's `mmArrow`). Authored-side verified on retail: `geometry/hudarrow{01,_blitz01,_cc01}.pkg` are PKG3 flat-XZ chevron meshes (tip −Z, authored origin near the tail = the rotation pivot) with 2 paint jobs × 1 shader each — family tile `s_hudarrow_{green,red,violet}` then the shared `s_hudarrow_yellow`, the exact ahead/behind pair RACE-6 documents; every `s_hudarrow_*` is a standard 8×8 Rgba8888 TEX the existing pipeline decodes. Landed: `mm2_app::navarrow` (new module — the arrow code moved out of `race.rs` like `racetime`/`oppind` precedents): `spawn_nav_arrow` picks the package by event family (`hudarrow_blitz01` Blitz, `hudarrow_cc01` Crash Course — already selected correctly though CC sessions still can't reach runtime, `hudarrow01` otherwise), rasterizes each of the first two paint jobs into an 80 px RGBA sprite (top-down projection, mesh origin at canvas centre, per-pixel `y` ordering, shader `paint*spj+offset` convention incl. negative-offset = untextured and non-triangle-strip skip — the `city.rs` semantics), binds through `city::load_image`/`read_pkg` (`absent:` `missing-pkg`/`unparseable-pkg`/`no-shaders`/`missing-texture`/`undecodable-texture`/`no-geometry` — never substitute art or a half-bound node); `update_nav_arrow` keeps the live contract — `UiTransform` rotation = signed bearing, `ImageNode` swaps ahead/behind sprites across the ±90° line, hidden on `Ordered`/stale/`Complete`/resolved/no-race and under the `H` gate while `NavArrowReport.facing` keeps recording (`arr=<stem>/<ahead|behind|off>` or `absent:<why>`, event sessions only). The dev needle/diamond (`NavArrowPart`, `NAV_AHEAD`/`NAV_BEHIND`) is deleted — the authored art replaces the stand-in. On-screen size/slot stay designed (DSN-8 updated; original presentation unrecovered — UNK-33). Evidence: `tests/navarrow.rs` +15 (binding incl. family-variant package pick, sprite rasterization/coverage, single-paint reuse, all four `absent` causes, ahead/behind swap + `facing` record, released-state hiding, `H` gate, synthetic-event `headless_smoke` `arr=` legs + dev-world field absence); `tests/race.rs` harness now binds a synthetic pkg so the bearing/target cycling legs exercise the real binding; `tests/hud.rs` stub updated to the sprite component set. Retail legs (`fnv1a64:e91e6cd4b2ae30d9`): sf `checkpoint:0` headless `arr=hudarrow01/ahead`, london `blitz:0` headless `arr=hudarrow_blitz01/ahead` (the family-variant pick on authored data), sf `checkpoint:0` windowed capture renders the authored green chevron at top-centre over the countdown field. Candidate pending external check. |
| F22-A.6 | implemented | F22-A.5 | HUD-2's remaining race instruments — the checkpoint list, the laps record (`Ordered`/Circuit) and the place indicator — bound to the authored `digitac_*_half` glyph set. Research correction first: the `texture/race_*` TGAs decode fine through `city::load_image` (plain 24bpp TGA 2.0, not the alpha-masked variant iteration 70 assumed) but visual inspection shows they are menu/results panels ("Laps", "Opponents", "Race Records", "Select Vehicle"), not in-race instrument labels — the instruments bind the half-size digit set and compose their own labels. Landed: `mm2_app::racestat` (new module): `spawn_race_stats` binds all ten `digitac_*_half` stems through `city::load_image` (any miss → `absent:missing-glyphs`, no half-bound cluster, never substitute art) and spawns a session-owned top-right translucent-plate column — `PLACE n/total` (live `live_order` standing, only while a field of ≥2 races — the `pos=` contract DSN-13 keeps), `LAP n/total` (spawned only under `Ordered`, resolved participants park at `laps/laps`), `CHECKPOINTS` listing every authored gate's 1-based index in the authored digits (cleared dims, the armed objective — `next` under `Ordered`, the arrow's `navigation_target` pick incl. `TargetSelection` under `AnyOrder` — lights warm), and a `FIN` entry under AnyOrder+finish that arms once every gate clears (RACE-7). `update_race_stats` composes off authoritative `RaceState`/`RaceProgress`/`live_order`/`navigation_target` — no parallel counters — hides on `Complete`/stale/no-race and under the `H` gate while `RaceStatReport` keeps recording (`sta=<glyphs>g/p<n>of<m>/l<n>of<m>/c<n>of<m>`, `-` per idle instrument, or `absent:<why>` — event sessions only, cruise/dev-world records bit-identical). Wired: `session.rs` spawn + teardown (`RaceStatReport` removed, nodes `SessionEntity`-stamped), `camera.rs` `HudNodes` retarget, main.rs + headless `Update` schedule after `drive_session`. Layout/labels/tints are a designed reading (DSN-54/UNK-34). Iteration-16 review repairs folded into A.5: the rasterizer now bounds-checks file-supplied strip indices (`absent:bad-index` instead of a panic on a hostile mod pkg) and selects the best-LOD chunk per stem with shadow/damage excluded (`pkg_to_parts` parity — `lod_split`); the `too_many_arguments` allow carries its why-comment. Evidence: `tests/racestat.rs` +18 (pair/gate slot composition, full/partial/missing glyph binding, Ordered/AnyOrder spawn shape, live place off `live_order`, cleared/armed/pending gate tints via real `advance` crossings, `FIN` arm, `Complete`/stale/cruise release, `H`-gate compose-under-hide, `SessionEntity` teardown, synthetic-event `sta=` legs, dev-world field absence); `tests/navarrow.rs` +2 (bad-index regression, best-LOD/shadow exclusion). Retail headless: sf `checkpoint:0` → `sta=10g/p3of7/-/c0of6` (matches `pos=3/7`), london `blitz:0` → `sta=10g/-/-/c0of3` (solo field hides place), sf `circuit:0` → `sta=10g/p5of5/l1of3/c0of9` (matches `lap=1/3`/`pos=5/5`). Windowed sf `checkpoint:0` `--frames 150 --screenshot` renders the top-right cluster with authored digits over the live field (`/tmp/f22a6_stats.png`, local). Candidate pending external check. |
| F22-B.1 | implemented | F22-A | Authored cockpit/dashboard view (HUD-1/HUD-3). Every stock `vp*` ships `geometry/<id>_dash.pkg` (flat quad parts in a shared cluster space: `dash`/`roof`/`speed_needle`/`tach_needle`/`damage_needle`/`gear_indicator`/`wheel`), `tune/<id>_dash.asnode` (`DashPos`/`RoofPos`/`WheelPos`, per-gauge `*Offset`/`*PivotOffset`, `*RotMin/Max` sweeps, `WheelFact`) and `tune/camera/<id>_dash.campovcs` (`camPovCS`: `Offset`/`ReverseOffset`/`TrackTo`/`Pitch`/FOV/near/far). Landed: `mm2_formats::dash` (`DashSpec`/`PovCamSpec` parsers over the tune grammar); `VehicleConfig::top_speed_mps` from authored `Trans.High` (mph→m/s; dev cars/trailers `None`); `mm2_app::dash` (`spawn_dash` — independent authored gates: camera needs `camPovCS`, cluster needs `asnode`+`pkg`, absence degrades to `absent:<why>`; `drive_dash` — needles by authored sweep radians against `top_speed_mps`/redline/damage fraction, wheel `steer/lock × WheelFact`, gear glyph by material swap across the pkg's repurposed paint-job slots `R,N,One…Six,D` — the recovered mechanism; `sync_dash_visibility` cockpit/exterior split; `cockpit_look` numpad glances + `ReverseOffset` back-look); `camera.rs` — `CameraMode::Cockpit`, marker-driven cycling that skips absent modes and never touches unmarked cameras, `V` dash toggle (`D` is steer-right under enhanced WASD — DSN-48); `DevOverrides::cockpit`/`--cockpit` flag with chase fallback; smoke `dash=<n>p/<cam|nocam>`. Evidence: sf/london `vpbug`+`vpbus` headless `dash=11p/cam`, windowed cockpit captures (gear window reads the engaged gear), 9 synthetic tests. Composition and `N`/`D` triggers remain designed/unrecovered (DSN-47/49, UNK-27/28). Iteration-63 review repair: `sync_dash_visibility` now restores only `CockpitHidden`-tagged children (never a `GlowPart`, and `detach_breaks` reclaims the tag on detach — a detached panel can no longer re-render beside its fragment, nor an unlit glow be re-shown); the effective `CameraMode` resolves before the session cameras spawn, so Cockpit without `camPovCS` (dashless car, dev world, persisted mode) falls back to an *active* chase camera, and a `C` press with zero marked cameras is a no-op; +1 dash test, +1 breakaway test (mid-cockpit detach), +1 session test (load-time fallback). Iteration-64 review repair: every "active camera" world-space consumer now reads `GlobalTransform`, not `Transform` — the cockpit camera is a vehicle child, so its `Transform` is the car-local eye offset. `drive_sky_dome` parked the dome near origin under Cockpit (clear-colour sky 1.3 km out), `active_cam_pose` reported car-local coords (moved to `mm2_app::camera`; HUD readout/screenshot/`--cam` round-trip restored), `apply_city_pvs` fed a bogus near-origin source (over-show only). The dome/pose/PVS picks also tightened to `WorldCamera3d` (a stray menu `Camera2d` can never be the world view). +3 regression tests (vehicle-child camera through all three consumers); windowed `--cockpit --spawn=-1300,63.5,250,0` capture renders the authored dome and a world-space `cam -1300.1,64.8,250.3,2,5` readout. |
| F22-B.2 | implemented | F22-B.1 | Rear-view mirror strip + restart rebinding (HUD-3/CTL-1): `BACKSPACE` is the documented rearview key and `F4` the documented restart — the dev build had borrowed BACKSPACE for restart, now corrected. Landed: `mm2_app::camera` — `MirrorCamera` marker, `RearView(bool)` resource (session-agnostic like `CameraMode`, so a restart respawns the strip armed), `spawn_mirror` (a rearward `Camera3d`, `order 2`, parented to the player vehicle — pitch/roll move it like a windshield mirror, teardown can never strand it; eye at the authored `camPovCS` `Offset` when present else a designed `chassis_size.y × 0.55` fallback; authored FOV/near/far with designed fallbacks; same `DistanceFog` as the other cameras), `mirror_input` (BACKSPACE toggles in `Playing`/`Countdown` only — pause/results/menu keep the key for Back), `drive_mirror` (`is_active` from `RearView`, suppressed under `CameraMode::Free`; top-centre `Viewport` strip at ⅓ × ⅛ of the physical window, write-on-diff; runs ungated by `capturing` so `--mirror` captures render it). `WorldCamera3d` excludes `MirrorCamera` — the strip is never the listener/PVS/dome/pose/billboard pick; `sync_dash_visibility` exempts it (`Camera3d` auto-`Visibility` would have been hidden under Cockpit — `is_active` is its only render gate). `DevOverrides::mirror`/`--mirror` is render-only and stays out of `record_eligibility`; headless records `mir=on|armed` on-activity only. Strip geometry, un-mirrored projection, fallback eye and Free suppression are designed — the original's mirror presentation is unrecovered (DSN-50/UNK-29). Evidence: `tests/mirror.rs` +7, `tests/session.rs` +1 (`f4_restarts_and_backspace_is_the_mirror` — BACKSPACE toggles the mirror not the session; F4 teardown/reload re-arms the strip), environment fog count honestly 2→3; retail sf headless `mir=on` + unchanged `pvs=`/`dash=`; windowed sf capture shows the rear view in the strip with the HUD `cam` pose still on the chase camera. |
| F22-B.3 | implemented | F22-B.2 | Authored chase near/far rigs + documented C chain (HUD-3): every stock `vp*` authors `tune/camera/<id>_{near,far}.camtrackcs` (`camTrackCS` records — mm2hook `camTrackCS : camCarCS` layout), and the documented chain is Chase-Near→Cockpit→Chase-Far on `C` — the dev build had Free occupying the far slot. Landed: `mm2_formats::camtrack` (`TrackCamSpec` parser over the tune grammar — optional `type:` tag, known scalar/vec fields, sparse records tolerated, malformed/short vectors treated absent, wrong root rejected, unknown fields preserved); `mm2_app::camera` — `CameraMode::ChaseFar`, `ChaseLens` (authored `Offset` boom/`TrackTo` aim/`MinDist`/`MaxDist`/`MinMaxOn`/`MinSpeed`/`MaxSpeed`/`CollideType`/FOV/near/far + `authored` flag; `ChaseLens::sized` is the designed size fallback), `load_track_cams` over the VFS, `TrackReport` (`trk=near+far|near|far|sized` on the smoke record — stock sessions only), marker-driven `C` cycling Chase-Near→Cockpit→Chase-Far→Free (Chase-Far skipped without a far record, Free appended as the dev extension — DSN-48's deviation repaired), `chase_follow` runs both chase modes with speed-window boom extension (designed reading) and `CollideType!=0` ray-cast occlusion pull-in excluding the player (margin/floor designed — UNK-36); session resolves invalid persisted modes (Cockpit w/o `camPovCS` or ChaseFar w/o far record → Chase) and spawns the chase cam on the active lens's projection; `input.rs` gates driving on every non-Free mode (cockpit/far steering repaired); `DevOverrides::far` + `--far` render-only diagnostic outside `record_eligibility`. Approach/steer/hill/reverse fields parse but stay unbound (UNK-36). Evidence: `tests/camtrack.rs` +9 (chain order incl. far-skip, lens binding + projection, speed window, occlusion pull-in/disabled, sized fallback, far-mode input, smoke `trk=` shapes); retail sf `vpbug` + london `vpbus` headless `trk=near+far`; windowed sf near/far captures show the authored booms differ. |
| F22-B.4 | implemented | F22-B.3 | Trailer-occlusion repair + cockpit near-clip repair + the AC05 atypical-vehicle capture leg (closes the F22-B.3 review's three non-blocking findings). The `CollideType` occlusion ray counted the player's *own* trailer as world geometry: vpsemi's authored `_near` boom (Offset z=7.73) lands inside its ~14 m trailer box, so the near view parked in the hitch gap on a close-up of the cab's rear wall and the far view's boom target (z=19.5) clamped behind the trailer's rear face — both verified broken on windowed retail captures. `chase_follow` now builds the exclusion set from `SpawnPoint.trailers` (the local rig) plus the player entity; other vehicles' trailers still occlude (designed reading — UNK-36; the original's occluder set is unrecovered). Separately, four `_dash.campovcs` records (vpsemi/vpcentury/vpcoop2k/vpvw_dune) author `CameraNear 3.0`, clipping the whole interior cluster ~1 m ahead of the eye into a bare windshield view; the cockpit camera now caps it at a designed 0.5 m (the authored value still reaches the mirror camera, where a high clip usefully hides the rig) — UNK-37. Evidence: `tests/camtrack.rs` 9→13 (`own_trailer_is_not_an_occluder`, `other_trailer_still_occludes`, `drive_views_steer_and_free_detaches`, `sized_lens_drives_the_fallback_boom` — the two test legs the B.3 review correctly noted were claimed but absent), `tests/dash.rs` +1 (near-cap), windowed retail captures on Apple Silicon/Metal: sf vpsemi near (flatbed deck + cab framed through the trailer footprint, was a wall of dark cab-rear), vpsemi far (whole rig from behind/above, was the trailer's rear wall), vpsemi+vpcentury `--cockpit` (full authored dashes, were bare windshields), vpbus near/far, vpcentury near, vpmoonrover near, vpsemi `--mirror`, and a dev-world wall pull-in (vpbug boom into the z=200 perimeter wall clamps at the authored margin). Candidate pending external check. |
| F22-B | active | F22-A | Split into B.1 (authored cockpit/dash view — implemented above), B.2 (rear-view mirror strip + F4 rebinding — implemented above), B.3 (authored chase near/far rigs + documented chain — implemented above) and B.4 (trailer-occlusion + cockpit near-clip repairs + atypical-vehicle captures — implemented above); the AC05 transition/reset legs and multi-resolution sweep (F22-C scope) plus the recovered-runtime semantics of `camTrackCS` dynamics (UNK-36) and `camPovCS` `CameraNear` (UNK-37) stay open. |
| F22-C | queued | F22-B | — |
