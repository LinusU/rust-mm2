...
**Next selected slice: F16-A remainder (app wiring), F15-B remainder,
...
| F15-C | queued | F15-B | — |
| F16-A | active | F01-A, F11-A | Split into A.1 (versioned profile store — implemented below). Remaining against the parent: app wiring (select/restore the active profile at startup, persist selections — the store API is the flow primitive, UI flows are F17 scope) and AC01's restart-isolation evidence, which needs F16-B's authoritative result→progress consumption to be observable. `players/` retail binaries (17 files, `player<N>.sav`/`*.cfg`) inspected — our own format, no compatibility claimed (DSN-15). |
| F16-A.1 | implemented | F01-A, F11-A | `mm2_game::profile` — `PlayerProfile` (id/name/`Difficulty` rank/`ProfileKind`/revision/`ProfileProgress`/`ProfileSelections`/unknown-field-preserving `extra`, serde JSON `version=1`) + `ProfileStore`: `driver-<n>` ids allocated max-suffix+1 (never reused), one `<id>.json` per profile plus `.bak` rotation, atomic tmp-write+`sync_all`+rename saves, `load` backup fallback reporting `recovered_from_backup`, corrupt files still listed by id and never deleted by a read, `version` mismatch rejected, size-bounded reads, id↔file-stem agreement checked, `active` marker for the selected profile, DRV-7 enforced (`LastProfile` on the final profile), `ProfileKind::Sandbox` gated by `records_progress()` for F16-B. `EventKey{city,table,stem}` keys progress by authored stem, not row index (spec req 4). `Difficulty`/`EventTableKind` gained serde derives (+`Ord` on the latter); `default_root()` resolves the OS data dir — installs untouched. Tests +15 (`tests/profile.rs`): round-trip, unknown-field preservation, isolation, duplicate names, invalid names, id non-reuse, corrupt-main→bak recovery, interrupted-write recovery, fully-corrupt reporting+preservation, version rejection, id-mismatch, last-profile refusal, delete scoping, active marker, sandbox gate. Candidate pending external check. |
