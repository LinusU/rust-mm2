...
| F21-B | queued | F21-A | — |
| F21-C | queued | F21-B | — |
| F22-A | active | F01-A, F02-B, F11-B | Split into A.1 (authored in-race HUD minimap — implemented), A.2 (the `I` opponent indicator — implemented below), A.3 (the `H` HUD master toggle — implemented below), A.4 (the authored stopwatch/countdown timer — implemented below) and A.5 (the authored `hudarrow*` compass arrow — implemented below); the race-HUD instrument remainder (HUD-2's checkpoint list/lap record/place indicator presentation over the dev telemetry line, plus the `race_*` label tiles) stays open. Dev HUD (speed/gear/RPM/grounded/cam pose) exists. |
