...
| F07-B | active | F07-A | Split: B.1 = RPM-driven engine loop rig for the local player (candidate at `280edb9`, pending external check), B.2 = opponent rigs + spatial emitters + camera listener (candidate at `eec3f62`, pending external check), B.3 = impact one-shot voices off the deduplicated `ImpactEvent` stream (checked), B.4 = surface skid/rolling loop voices off wheel contacts (below). Remaining: ambient-traffic engines (different cardata grammar — `AmbientEngine`), clutch sample trigger, siren programs, scrape/continuous-impact semantics, weather→surface-variant binding (dry bound unconditionally — UNK-25), AC02/AC05 scripted + audible evidence. |
