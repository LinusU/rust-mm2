...
implemented F17-A.6: the menu's condition-options screen —
...
| F18-A | active | F01-B, F06-B | Split: A.1 (preset inventory — implemented below), A.2 (preset→Bevy lighting binding + session-legal selection — implemented below). Remaining: `.sky` dome/fog mapping, `.cpvs` PVS consumption, precipitation/wetness/audio and condition replication (F18-B/C scope — see UNK-24). Menu weather/tod selection landed in F17-A.6 through `SessionConfig::customization`. |
