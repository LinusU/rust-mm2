...
(race-HUD instruments over the dev line; A.1 landed the authored
minimap), F17-A remainder
...
semantics unverified))** — the latest iteration landed F22-A.2
(iteration 67): the documented `I` opponent indicator (HUD-3/CTL-1) —
a session-owned pool of markers over each non-local `Player`
participant, bound per-frame in entity order so stale/invalid
participants free their slot the same update; `I` toggles in
`Playing`/`Countdown` only and the toggle is session-agnostic.
Markers bind authored `geometry/hudmap_tri.pkg` through the VFS
(`absent` reported, never substituted), stand upright apex-down over
the authored collider/`chassis_size` roof, yaw-face the
`WorldCamera3d` camera, and paint from the same authored opponent
palette the minimap's tri pool cycles — the original's indicator
presentation is unrecovered, so shape/extent/lift/palette/default are
designed readings (DSN-51/UNK-30). Headless records report
`ind=<on|off>/<pool>m/<bound>b`; the smoke app's Update tuple hit
Bevy's arity limit so the two systems run in their own slot like the
windowed app. Verified by 5 new oppind tests (phase gating, authored
binding, per-car roof heights, local-car exclusion, Remote control,
despawn/vehicle-drop release, hidden-while-off demand count, missing
pkg), a synthetic-event smoke leg (`ind=on/1m/1b`), retail sf
checkpoint:0 headless `ind=on/6m/6b` and a windowed countdown capture
showing painted arrows over the staged field. Before that it repaired
...
| F21-B | queued | F21-A | — |
| F21-C | queued | F21-B | — |
| F22-A | active | F01-A, F02-B, F11-B | Split into A.1 (authored in-race HUD minimap — implemented) and A.2 (the `I` opponent indicator — implemented below); the race-HUD instrument remainder (HUD-1/HUD-2's dashboard/race set over the dev telemetry line, plus the documented `H` HUD toggle) stays open. Dev HUD (speed/gear/RPM/grounded/cam pose) exists. |
