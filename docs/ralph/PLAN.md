...
F15-B remainder (`unkFlag`/`cornerBrakingThreshold`/
`weirdPathfinding` consumption — semantics unverified, research-gated),
...
| F14-A | active | F02-B, F11-B | Split into A.1 (binding honesty + lap evidence — implemented below), A.2 (AC06 representative-playability matrix + the densify gate-coverage repair its analysis surfaced — implemented below), A.3 (route-bound Ordered AI progress over authored lines that miss gates — implemented below), A.4 (Professional scripted+parked matrix — implemented below). London circuit0–11, SF circuit0–11 authored data present (circuit11 partial: opp/pathset only, no .aimap). SF `cir1–9` are the circuit events' start grids under a short stem — aliased to `circuit<N>` since F11-B.2 (WPT-3). Remaining: event `.aimap` `[Exceptions]`/density scoping needs consumers (F15/F10 scope); traversal-bound stalls under the new progress model (london-2 @gate0-bind ~700 m, london-5 @gate2-bind ~1050 m, sf-7 pack @gate5-bind ~985 m — fields never drive far enough to earn the bind, F15-B controller class); hold-driver legs + AC04/AC05/AC06 promotion remain open. |
