...
minimap, A.2 the `I` indicators, A.3 the `H` master toggle), F17-A remainder
...
semantics unverified))** — the latest iteration landed F22-A.3
...
| F21-B | queued | F21-A | — |
| F21-C | queued | F21-B | — |
| F22-A | active | F01-A, F02-B, F11-B | Split into A.1 (authored in-race HUD minimap — implemented), A.2 (the `I` opponent indicator — implemented below) and A.3 (the `H` HUD master toggle — implemented below); the race-HUD instrument remainder (HUD-2's checkpoint list/lap record/place/stopwatch/countdown presentation over the dev telemetry line) stays open. Dev HUD (speed/gear/RPM/grounded/cam pose) exists. |
