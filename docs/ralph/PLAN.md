...
**Next selected slice: F16-B remainder (availability-query leg or
F16-C evidence run), F15-B remainder, F13-B/F14-B remainders, or
F11-C remainder** — the latest iteration repaired F16-B.1's first
...
| F03-C | implemented | F03-B | Split: C.1 (lateral placement audit across all channels — implemented below). Owed: sampled original locations beyond the stamp-position audit (asymmetric-prop yaw, UNK-20 expansion density), race cleanup, mod replacement end-to-end. |
