...
detachment if original, impairment, C&R healing driver),
...
| F05-B | active | F05-A | Split: B.1 (runtime impact→damage application + disabled outcomes), B.2 (authored `vehstuck` detection + in-place recovery), B.3 (authored breakaway detachment — impact-driven reading, DSN-21) and B.4 (authored `vehgyro` consumption — designed application reading, DSN-22) and B.5 (water/out-of-bounds recovery — designed policy end to end, DSN-23) implemented below. Remaining: visual tiers (smoke pivots, `TextelDamageRadius` decals, `DoublePivot`/`MirrorPivot`), damage-driven detachment if the original uses it (UNK-13), impairment short of destruction, C&R healing driver (DMG-4), replication. |
