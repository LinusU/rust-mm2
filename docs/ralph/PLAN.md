...
semantics unverified))** — the latest iteration repaired the F21-A.1
...
| F22-B | active | F22-A | Split into B.1 (authored cockpit/dash view — implemented above), B.2 (rear-view mirror strip + F4 rebinding — implemented above) and B.3 (authored chase near/far rigs + documented chain — implemented above); the atypical-vehicle framing/interpolation validation remainder (F22-AC05) and the recovered-runtime semantics of `camTrackCS` dynamics (UNK-36) stay open. |
| F22-C | queued | F22-B | — |
