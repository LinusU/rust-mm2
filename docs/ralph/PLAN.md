...
`ImpactsTable` texel damage + `asLineSparks` sparks, damage-driven
detachment if original, impairment — MM2Hook's
`PhysicalEngineDamage` documents smoke↔torque coupling — C&R
healing driver),
...
| F05-B | active | F05-A | Split: B.1 (runtime impact→damage application + disabled outcomes), B.2 (authored `vehstuck` detection + in-place recovery), B.3 (authored breakaway detachment — impact-driven reading, DSN-21), B.4 (authored `vehgyro` consumption — designed application reading, DSN-22), B.5 (water/out-of-bounds recovery — designed policy end to end, DSN-23) and B.6 (authored engine-smoke visual tier — designed emission policy, DSN-24) implemented below. Remaining: `TextelDamageRadius`/`ImpactsTable` texel damage + `asLineSparks` sparks (UNK-13), damage-driven detachment if the original uses it (UNK-13), impairment short of destruction (MM2Hook `PhysicalEngineDamage` documents smoke↔torque coupling — shape unrecovered), C&R healing driver (DMG-4), replication. |
