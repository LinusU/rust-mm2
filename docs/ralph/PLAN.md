...
(skids/ambient engines/clutch/siren/scrape + audible
evidence))** — the latest iteration landed F07-B.3: impact
one-shot voices off the deduplicated `ImpactEvent` stream —
the session loads the player-side `default_impacts.csv` into
`ImpactAudio` (absent/malformed → no resource), and each
event's vehicle participants each earn one bounded
(`MAX_IMPACT_VOICES` 12) `PlaybackMode::Despawn` voice:
struck-side `AudioId`→category `ID` with id-0 `WALL` fallback,
`severity × striker mass` picks the force band, `frequency`
weights and authored volume range draws the gain — all
designed readings under UNK-25; spatial emitters at the
contact for everyone but the local player (DSN-37/DSN-38).
