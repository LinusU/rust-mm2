...
remaining box-yield/spawn-occupancy/collision scope** — the
...
| F10-B | active | F10-A | B.1 obstruction response + B.2 authored junction rules + B.3 bounded stuck recovery implemented above. Remaining: right-of-way/yielding between vehicles inside the junction, queue-through-intersection priority, spawn-vs-spawn occupied-space checks, multiplayer union-of-interest bubbles, signal-prop rendering, original junction timing (UNK-12), F10-AC evidence vs the spec. |
| F10-C | queued | F10-B | — |
