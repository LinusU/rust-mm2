...
| F16-B | active | F16-A | Split into B.1 (reward import + authoritative consumption — implemented below) and B.2 (availability derived state — implemented below). Remaining against the parent: availability *enforcement* is F17's menu flow (the query exists and warns on locked `--event` launches); vehicle/paint selectability queries (`unlocks` set → roster) for F17's garage; `vpmoonrover` has no authored rule (UNK-3); Pro points (DRV-4/UNK-8) unimplemented; AC01's two-profile restart observation and AC06's deliberate-delete UI are F16-C/F17 evidence. |
