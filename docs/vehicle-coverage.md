...
a fallback, and its `TWHL0`/`TWHL1` parts are decorative 5 cm detail
excluded from the physics rig; `vpford` — metadata declares 4 paints,
the model has 5, so 4/5 validated). `validate-cars --strict` exits 2
on those warnings; `handling --strict` exits 0 — every ready car sits
inside the arcade envelope.
...
| vpcentury | Freightliner Century | 4/4 | 6 | 3500 | 1.87 | ok (+4 warn) |
...
then `ResetVehicle` and re-settled): **21/21 ok on both cities** —
...
6/6 + trailer 4/4, `vpsemi` 4/4 + trailer 4/4, hitch-anchor gap
0.00 m after spawn and reset on both.
...
- **The moon rover settles slowly but correctly.** At the probe's
  ~1.5 m-high London spawn it wallowed for >6 s — mid-settle it even
  grazed a hull corner — before grounding 6/6 with no contacts. The
  clearance leg now settles adaptively (until still, capped at 15 s)
  rather than reading a transient.
